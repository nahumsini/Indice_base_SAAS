...
LOCAL_AUTH_MFA_BYPASS_ENABLED ?= true
LOCAL_EMAIL_ENABLED ?= false
LOCAL_WEB_PUBLIC_URL ?= http://$(FRONTEND_HOST):$(FRONTEND_PORT)
LOCAL_SIGNUP_EMAIL_VERIFICATION_ENABLED ?= false
LOCAL_KIOSK_EMPLOYEE_CENTER_ENABLED ?= true
LOCAL_KIOSK_GLOBAL_CENTER_ENABLED ?= true
LOCAL_KIOSK_MULTI_DASHBOARD_ENABLED ?= true
LOCAL_KIOSK_ADAPTER_PETTY_CASH_ENABLED ?= true
LOCAL_KIOSK_ADAPTER_PAYABLES_ENABLED ?= true
...
dev: prepare ## Start the full local dev stack
	...
	APP_AUTH_LOCAL_MFA_BYPASS_ENABLED="$(LOCAL_AUTH_MFA_BYPASS_ENABLED)" \
	APP_EMAIL_ENABLED="$(LOCAL_EMAIL_ENABLED)" \
	APP_WEB_PUBLIC_URL="$(LOCAL_WEB_PUBLIC_URL)" \
	SERVER_ADDRESS="127.0.0.1" \
	APP_BILLING_SIGNUP_EMAIL_VERIFICATION_ENABLED="$(LOCAL_SIGNUP_EMAIL_VERIFICATION_ENABLED)" \
	KIOSK_EMPLOYEE_CENTER_ENABLED="$(LOCAL_KIOSK_EMPLOYEE_CENTER_ENABLED)" \
	KIOSK_GLOBAL_CENTER_ENABLED="$(LOCAL_KIOSK_GLOBAL_CENTER_ENABLED)" \
	KIOSK_MULTI_DASHBOARD_ENABLED="$(LOCAL_KIOSK_MULTI_DASHBOARD_ENABLED)" \
	KIOSK_ENGINE_ADAPTER_PETTY_CASH_ENABLED="$(LOCAL_KIOSK_ADAPTER_PETTY_CASH_ENABLED)" \
	KIOSK_ENGINE_ADAPTER_PAYABLES_ENABLED="$(LOCAL_KIOSK_ADAPTER_PAYABLES_ENABLED)" \
	./mvnw spring-boot:run -P$(SPRING_PROFILE) \
		-Dspring-boot.run.profiles="local,$(SPRING_PROFILE)" \
