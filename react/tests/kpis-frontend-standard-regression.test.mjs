...
test('el diagnóstico Índice usa los contratos ejecutivos vigentes, cuatro sectores y calidad fail-closed', () => {
  ...
  const compactOverviewSource = readFileSync(resolve(moduleRoot, 'KPIs/CompactDiagnosisOverview.tsx'), 'utf8');
  const crossKpiSource = readFileSync(resolve(moduleRoot, 'KPIs/CrossKpiWorkspace.tsx'), 'utf8');
  const compactTranslationsSource = readFileSync(resolve(moduleRoot, 'KPIs/compactDiagnosisTranslations.ts'), 'utf8');
  ...
  assert.match(panelSource, /variant="views"/);
  assert.ok(panelSource.indexOf('<IndiceTitleBar') < panelSource.indexOf('<IndiceWorkspaceNavigation<DiagnosisViewId>'));
  assert.ok(panelSource.indexOf('<IndiceWorkspaceNavigation<DiagnosisViewId>') < panelSource.indexOf('<DiagnosisFilters'));
  assert.doesNotMatch(panelSource, /DropdownMenu/);
  assert.match(panelSource, /useWorkspaceNavigationMemory/);
  assert.match(panelSource, /activeView: 'analysis'/);
  assert.match(panelSource, /CompactDiagnosisOverview/);
  ...
  assert.match(panelSource, /CrossKpiWorkspace/);
  ...
  assert.match(panelSource, /compactCopy\.navigation\.diagnosis/);
  assert.match(panelSource, /compactCopy\.navigation\.crossings/);
  assert.match(panelSource, /compactCopy\.navigation\.swot/);
  assert.match(panelSource, /compactCopy\.navigation\.sources/);
  ...
  assert.match(crossKpiSource, /data-testid="cross-kpi-workspace"/);
  assert.match(crossKpiSource, /evidenceFindingCodes/);
  assert.match(crossKpiSource, /compactCopy\.crossing\.evidence/);
  assert.match(crossKpiSource, /selectedCodes/);
  assert.match(crossKpiSource, /current\.length >= 4/);
  assert.match(crossKpiSource, /aria-pressed=\{selected\}/);
  assert.match(compactOverviewSource, /data-testid="compact-diagnosis-overview"/);
  assert.match(compactOverviewSource, /criticalCount/);
  assert.match(compactOverviewSource, /missingCount/);
  assert.match(compactOverviewSource, /specializedTools/);
  assert.match(compactOverviewSource, /showAllSignals/);
  assert.match(compactOverviewSource, /data-testid="decision-summary"/);
  assert.match(compactOverviewSource, /data-testid="sector-comparison"/);
  assert.match(compactOverviewSource, /data-testid="analysis-tools"/);
  assert.doesNotMatch(compactOverviewSource, /MaturityJourney/);
  ...
  for (const locale of ['es-MX', 'es-CO', 'en-US', 'en-CA', 'fr-CA', 'pt-BR', 'ko-CA', 'zh-CA']) {
    ...
    assert.match(compactTranslationsSource, new RegExp(`'${locale}'`));
