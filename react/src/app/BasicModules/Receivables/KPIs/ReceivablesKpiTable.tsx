...
import { SortableReceivablesHead } from '../components/ReceivablesSortableTable';
...
export function normalizeTablePreferences(value: unknown): Record<string, KpiTablePreference> {
  ...
  return Object.fromEntries(Object.entries(value).filter(([key]) => ['units', 'customers', 'accounts', 'payments', 'aging', 'trend'].includes(key)).map(([key, raw]) => {
...
export function ReceivablesKpiTable({ model, copy, locale, preference = defaultTablePreference, onPreference, onSelect }: {
  model: KpiTableModel; copy: ReceivablesKpiCopy; locale: string; preference?: KpiTablePreference;
  ...
  return <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
    ...
    <div className="hidden overflow-x-auto md:block"><table className="w-full text-left text-sm"><thead className="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900/50"><tr>{model.columns.map((column, index) => model.sortable.includes(index)
      ? <SortableReceivablesHead key={index} column={{ id: String(index), label: column, visible: true }} onSort={onSort} sortState={{ columnId: String(sortIndex), direction: preference.sortDirection }} />
    ...
    {model.rows.length === 0 ? <div className="p-5"><p className="text-sm text-slate-500">{copy.empty}</p></div> : null}
    <DataTablePagination currentPage={pagination.currentPage} itemLabel={model.title} onPageChange={pagination.onPageChange} onPageSizeChange={pagination.onPageSizeChange} pageEnd={pagination.pageEnd} pageSize={pagination.pageSize} pageSizeOptions={pagination.pageSizeOptions} pageStart={pagination.pageStart} totalCount={pagination.totalCount} totalPages={pagination.totalPages} />
  </section>;
}
