...
import { formatAccountingCurrency, getTypeBadgeColor, getTypeLabel } from '../accountingAccounts.utils';
...
type AccountingAccountsTableProps = {
  ...
  visibleColumns: AccountingColumnKey[];
...
export function AccountingAccountsTable({
  ...
  const tableColumns = useMemo(() => accountingHeaders.filter(header => visibleColumns.includes(header.key)), [visibleColumns]);
        ...
        <table className="min-w-full">
          <thead className="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900/60">
            <tr>
              ...
              <th className="px-5 py-4 text-center text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400" style={{ width: defaultAccountingColumnWidths.actions, minWidth: defaultAccountingColumnWidths.actions }}>Acciones</th>
            </tr>
          </thead>
    ...
    <tr className="transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/50">
      {tableColumns.map(columnKey => (
        <td key={columnKey} className="px-5 py-4 text-sm text-slate-700 dark:text-slate-300" style={{ width: defaultAccountingColumnWidths[columnKey], minWidth: defaultAccountingColumnWidths[columnKey] }}>
          {renderAccountCell(columnKey, account, unitOptions, businessOptions)}
        </td>
      ))}
      <td className="px-5 py-4 text-center" style={{ width: defaultAccountingColumnWidths.actions, minWidth: defaultAccountingColumnWidths.actions }}>
        <div className="mx-auto inline-flex items-center justify-center gap-2 rounded-[22px] border border-slate-200 bg-white px-3 py-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <button type="button" onClick={() => onEdit(account)} className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-[#147514]/20 bg-[#147514]/5 text-[#147514] transition hover:-translate-y-0.5 hover:bg-[#147514]/12 hover:shadow-sm dark:border-[#147514]/30 dark:bg-[#147514]/10" title="Editar">
    ...
    <tr>
      <td colSpan={colSpan} className="px-6 py-12 text-center">
        <div className="flex flex-col items-center justify-center text-slate-500 dark:text-slate-400">
          ...
          <p className="text-lg font-semibold">No se encontraron cuentas contables</p>
