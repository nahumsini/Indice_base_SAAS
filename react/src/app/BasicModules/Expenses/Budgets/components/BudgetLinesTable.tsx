        ...
        <section role="alert" className="flex flex-col gap-3 border-b border-amber-200 bg-amber-50 px-5 py-4 dark:border-amber-900/60 dark:bg-amber-950/25 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium text-amber-950 dark:text-amber-100">{t.budgets.messages.loadErrorTitle}</p>
              ...
              <p className="mt-1 break-words text-xs font-medium text-amber-700/80 dark:text-amber-300/80">{errorMessage}</p>
            </div>
          </div>
          {onRetry ? (
            <button type="button" onClick={onRetry} className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-amber-300 bg-white px-4 text-sm font-medium text-amber-800 transition hover:bg-amber-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/40 dark:border-amber-800 dark:bg-slate-900 dark:text-amber-200 dark:hover:bg-amber-950/50">
              ...
              <tr>
                <td colSpan={visibleColumns.length + 1} className="px-6 py-16 text-center">
                  <div className="mx-auto max-w-md rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 dark:border-slate-700 dark:bg-slate-800/70">
                    <p className="text-base font-medium text-slate-900 dark:text-white">{t.budgets.messages.emptyTitle}</p>
                    <p className="mt-2 text-sm font-medium leading-6 text-slate-500 dark:text-slate-400">{t.budgets.messages.emptyMessage}</p>
                  </div>
                </td>
              </tr>
  ...
  return (
    <section className="border-b border-[#147514]/20 bg-[#147514]/10 px-4 py-3 dark:border-[#147514]/40 dark:bg-[#147514]/15">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <span className="w-fit rounded-full border border-[#147514]/30 bg-white px-3 py-1 text-sm font-medium text-[#147514] dark:bg-slate-800 dark:text-emerald-200">
...
function renderBudgetCell(
  ...
  switch (key) {
    ...
    case 'concept':
      return (
        <div className="max-w-[240px]">
          <p className="line-clamp-2 font-medium leading-5 text-slate-800 dark:text-slate-100">{budgetLine.concept}</p>
    ...
    case 'providerName':
      return <span className="line-clamp-2 max-w-[180px] font-medium">{budgetLine.providerName || '-'}</span>;
    case 'accountingAccount':
      return <span className="line-clamp-2 max-w-[200px] font-medium">{budgetLine.accountingAccount || '-'}</span>;
    case 'plannedAmount':
    case 'total':
    ...
    case 'dueDate':
      return <span className="font-medium text-slate-700 dark:text-slate-200">{formatDate(budgetLine.dueDate)}</span>;
    case 'actions':
      return (
...
function StackedText({ primary, secondary }: { primary: string; secondary?: string }) {
  return (
    <div className="max-w-[160px]">
      <p className="line-clamp-2 font-medium text-slate-800 dark:text-slate-100">{primary}</p>
...
function MoneyCell({ amount, currency, tone }: { amount: number; currency: string; tone: 'amber' | 'base' | 'blue' | 'green' | 'red' }) {
  ...
  return (
    <span className={`inline-flex min-w-[112px] justify-end rounded-xl border px-3 py-2 text-sm font-medium ${toneClass}`}>
