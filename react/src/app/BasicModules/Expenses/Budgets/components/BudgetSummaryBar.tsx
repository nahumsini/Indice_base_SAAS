...
export function BudgetSummaryBar({ expenses }: BudgetSummaryBarProps) {
  ...
  const accountMetrics = buildAccountMetrics(expenses, safeTotal);
    ...
    <div className="rounded-2xl border border-slate-200 bg-white px-5 py-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <Metric icon={<CircleDollarSign className="h-4 w-4" />} label="Total estimado" value={formatCurrency(safeTotal)} />
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-bold text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
            {expenses.length} linea{expenses.length !== 1 ? 's' : ''} presupuestada{expenses.length !== 1 ? 's' : ''}
          </span>
        </div>
      </div>
    </div>
...
function buildAccountMetrics(expenses: Expense[], safeTotal: number): BudgetAccountMetric[] {
  const groupedAccounts = expenses.reduce<Record<string, { amount: number; count: number }>>((groups, expense) => {
    const account = expense.accountingAccount?.trim() || 'Sin cuenta contable';
    const current = groups[account] ?? { amount: 0, count: 0 };
    groups[account] = {
...
function AccountMetricItem({ metric }: { metric: BudgetAccountMetric }) {
  return (
    <div className="flex min-w-fit items-center gap-2 text-sm text-slate-600 dark:text-slate-400" title={`${metric.label}: ${metric.count} linea${metric.count !== 1 ? 's' : ''}`}>
