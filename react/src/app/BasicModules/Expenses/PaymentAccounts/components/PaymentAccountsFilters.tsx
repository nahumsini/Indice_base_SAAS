...
export function PaymentAccountsFilters({
  ...
  return (
    <IndiceFilterBar gridClassName="md:grid-cols-[minmax(280px,1.4fr)_repeat(2,minmax(0,1fr))]" summary={t.common.results(filteredCount)} title={t.filters.title}>
      <IndiceFilterSearch label={t.common.search} onClear={() => onSearchChange('')} onValueChange={onSearchChange} placeholder={t.paymentAccounts.filters.searchPlaceholder} tone="green" value={searchTerm} />
      <IndiceFilterSelect label={t.paymentAccounts.filters.type} value={typeFilter} onValueChange={onTypeChange} options={typeOptions} tone="green" />
      <IndiceFilterSelect label={t.filters.status} value={statusFilter} onValueChange={onStatusChange} options={statusOptions} tone="green" />
    </IndiceFilterBar>
  );
}
