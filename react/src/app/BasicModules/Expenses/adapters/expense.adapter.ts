...
export const toFinanceExpense = (expense: Expense, companyId = 'mock-company'): FinanceExpense => {
  ...
  return {
    ...
    reference: expense.reference ?? expense.folio,
...
export const toExpense = (
  ...
  return {
    ...
    reference: asString(customFields.reference, expense.folio),
    budgetLineId: expense.budgetLineId ? String(expense.budgetLineId) : asString(customFields.budgetLineId, undefined),
    ...
    originFund: expense.originFund ? { ...expense.originFund, id: String(expense.originFund.id) } : undefined,
    accountingPosted: expense.accountingPosted,
    purchaseOrderReceived: expense.purchaseOrderReceived,
...
export const toExpenseApiRequest = (expense: Expense): ExpenseApiRequest => ({
  ...
  purchaseOrderId: numericId(expense.purchaseOrderId) ?? null,
  budgetLineId: numericId(expense.budgetLineId) ?? numericId(expense.id.startsWith('budget-line-') ? expense.id.replace('budget-line-', '') : undefined) ?? null,
