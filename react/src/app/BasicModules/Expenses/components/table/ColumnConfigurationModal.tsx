      ...
      <div className="flex h-[min(86vh,820px)] max-h-[calc(100vh-3rem)] w-full max-w-[900px] flex-col overflow-hidden rounded-[28px] border border-slate-200/80 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.32)]">
        <div className="flex shrink-0 items-center justify-between bg-[#147514] px-6 py-5 text-white">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm font-medium text-white/85">{description ?? t.expenses.columnModalDescription}</p>
          </div>
          ...
          <div className="shrink-0 border-b border-slate-200 bg-white px-6 py-5">
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
              ...
              <div className="grid gap-2 sm:grid-cols-2 lg:min-w-[320px]">
                <ToolbarButton icon={<Eye className="h-4 w-4" />} onClick={onShowAllColumns}>{t.columnModal.selectAll}</ToolbarButton>
                <ToolbarButton icon={<EyeOff className="h-4 w-4" />} onClick={onHideOptionalColumns}>{t.common.deselectAll}</ToolbarButton>
              </div>
            </div>
        ...
        <div className="flex shrink-0 items-center justify-end gap-3 bg-[#147514] px-6 py-4">
          <button type="button" onClick={onClose} className="h-12 rounded-2xl border border-white/35 bg-white/10 px-7 text-sm font-extrabold text-white transition hover:bg-white/18">{t.columnModal.cancel}</button>
