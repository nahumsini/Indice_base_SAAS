...
export async function loadExpenseKpiAggregates(queries: KpiMonetaryBatchQuery[], isActive = () => true) {
  ...
  for (let offset = 0; offset < queries.length && isActive(); offset += 100) {
    ...
    if (batch.some(query => !response[query.key])) throw new Error('Incomplete expense KPI response');
    Object.assign(results, response);
  }
  return results;
}

export function useExpenseKpiAggregates(queries: KpiMonetaryBatchQuery[], sourceRevision: object, enabled = true) {
  ...
  useEffect(() => {
    if (!enabled) return;
    let active = true;
    loadExpenseKpiAggregates(JSON.parse(key), () => active)
      .then(data => { if (active) setSnapshot({ key, revision: sourceRevision, data, error: null }); })
      .catch(error => { if (active) setSnapshot({ key, revision: sourceRevision, data: {}, error: error instanceof Error ? error : new Error('Expense KPI request failed') }); });
    return () => { active = false; };
