...
export function ExpenseKpiAnalysis({ active, copy, money, groupReady, budgetReady, taxReady, driverReady, hasCutoff, paymentMix, trend, budgetRows, budgetLabels, aging, forecast, tax, drivers, onDriver }: {
  ...
  return <div className="grid gap-6">
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      <SectionCard title={copy.paymentMix} subtitle={copy.mixHelp}><DataState active={active} ready={groupReady} empty={!paymentMix.length} copy={copy}><ResponsiveContainer><PieChart><Pie data={paymentMix} dataKey="value" nameKey="name" innerRadius={70} outerRadius={95}>{paymentMix.map(row => <Cell key={row.key} fill={row.color} />)}</Pie><Tooltip formatter={tooltip} /><Legend /></PieChart></ResponsiveContainer></DataState></SectionCard>
