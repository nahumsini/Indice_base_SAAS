import { CheckCircle2, Copy, KeyRound, Pencil, Trash2 } from 'lucide-react';
...
type ProviderRowActionsProps = {
  ...
  onEditProvider: () => void;
  onManageAccess: (providerId: string) => void;
};

const buttonBase = 'inline-flex h-10 w-10 items-center justify-center rounded-2xl border transition-all hover:-translate-y-0.5 hover:shadow-sm dark:border-slate-600 dark:bg-slate-800/80';
...
export function ProviderRowActions({
  ...
  return (
    <div className="inline-flex items-center justify-center gap-2 rounded-[22px] border border-slate-200 bg-white px-3 py-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <ActionButton label="Acceso CxP" className="border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 dark:border-emerald-900/60 dark:bg-emerald-950/60 dark:text-emerald-300" onClick={() => onManageAccess(providerId)}><KeyRound className="h-4 w-4" /></ActionButton>
