...
export function toFrontendProduct(row: ApiRow): SalesCatalogItem {
  ...
  return {
    ...
    readyForSales: typeof row.readyForSales === 'boolean' ? row.readyForSales : undefined,
    salesReadiness: toStringValue(row.salesReadiness) as SalesCatalogItem['salesReadiness'] || undefined,
    salesReadinessReasons: Array.isArray(row.salesReadinessReasons)
      ? row.salesReadinessReasons.map((reason) => toStringValue(reason)) as SalesCatalogItem['salesReadinessReasons']
      : undefined,
