...
export function RouteSalesMultiKioskWorkspace({
  ...
  const contactForm = (
    <section className="rounded-2xl border border-rose-200 bg-rose-50/60 p-4 dark:border-rose-900/60 dark:bg-rose-950/20">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-semibold text-slate-950 dark:text-white">Nuevo cliente</p>
      ...
      <details className="group mt-4 overflow-hidden rounded-xl border border-rose-200 bg-white dark:border-rose-900/60 dark:bg-slate-950">
        <summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-3 py-2 outline-none focus-visible:ring-4 focus-visible:ring-rose-100 [&::-webkit-details-marker]:hidden">
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-sm font-semibold text-slate-950 dark:text-white">Datos fiscales</span>
            <span className="block text-xs text-slate-500">Se guardan en la ficha real del cliente para facturación.</span>
          </span>
          <span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-semibold ${fiscalDataCount ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-200' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-300'}`}>
  ...
  return (
    <div className="min-w-0 max-w-full space-y-3 overflow-x-hidden" data-route-sales-workspace>
      ...
      <section className="overflow-hidden rounded-2xl border border-rose-200 bg-white shadow-sm dark:border-rose-900/60 dark:bg-slate-950">
        <div className="flex items-center gap-3 bg-gradient-to-r from-[#E85D52] to-[#c94840] px-4 py-3 text-white sm:px-5">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-semibold">Venta en ruta</p>
        ...
        <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-slate-800">
          <button type="button" className={`min-h-14 px-2 py-2 text-xs font-semibold ${tab === 'sell' ? 'bg-rose-50 text-[#C94840] dark:bg-rose-950/30 dark:text-rose-200' : 'text-slate-500 dark:text-slate-400'}`} onClick={() => setTab('sell')}><ShoppingBag className="mx-auto mb-1 h-4 w-4" />Vender</button>
          <button type="button" className={`min-h-14 px-2 py-2 text-xs font-semibold ${tab === 'sales' ? 'bg-rose-50 text-[#C94840] dark:bg-rose-950/30 dark:text-rose-200' : 'text-slate-500 dark:text-slate-400'}`} onClick={() => setTab('sales')}><PackageCheck className="mx-auto mb-1 h-4 w-4" />Mis ventas</button>
          <button type="button" className={`min-h-14 px-2 py-2 text-xs font-semibold ${tab === 'customers' ? 'bg-rose-50 text-[#C94840] dark:bg-rose-950/30 dark:text-rose-200' : 'text-slate-500 dark:text-slate-400'}`} onClick={() => setTab('customers')}><UsersRound className="mx-auto mb-1 h-4 w-4" />Clientes</button>
        </div>
      </section>
        ...
        <div className="space-y-3">
              ...
              {(['Cliente', 'Productos', 'Cobro', 'Revisar'] as const).map((label, index) => {
                ...
                return (
                  <div key={label} className="min-w-0 text-center">
                    <span className={`mx-auto grid h-7 w-7 place-items-center rounded-full text-xs font-bold ${complete ? 'bg-emerald-500 text-white' : active ? 'bg-[#E85D52] text-white' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-300'}`}>{complete ? <Check className="h-4 w-4" /> : number}</span>
          ...
          {createdSale ? (
            <section className="rounded-2xl border border-emerald-200 bg-white p-5 text-center dark:border-emerald-900/60 dark:bg-slate-950">
              ...
              <h3 className="mt-3 text-lg font-semibold text-slate-950 dark:text-white">Venta terminada</h3>
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Folio {createdSale.sale_number}</p>
              <p className="mt-3 text-2xl font-bold text-slate-950 dark:text-white">{money(createdSale.total_amount, createdSale.currency, locale)}</p>
          ...
          {!createdSale && step === 1 ? (
            <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
              <div className="flex items-start justify-between gap-3">
                <div><h3 className="font-semibold text-slate-950 dark:text-white">¿A quién le vendes?</h3><p className="mt-1 text-xs text-slate-500">Sólo ves clientes de tu cartera.</p></div>
              ...
              <div className="grid max-h-[48vh] gap-2 overflow-y-auto sm:grid-cols-2">
                {visibleContacts.map(contact => (
                  <button key={contact.id} type="button" className={`rounded-xl border p-3 text-left transition ${contact.id === contactId ? 'border-[#E85D52] bg-rose-50 ring-2 ring-rose-100 dark:bg-rose-950/30 dark:ring-rose-950' : 'border-slate-200 hover:border-rose-300 dark:border-slate-700'}`} onClick={() => setContactId(contact.id)}>
                    <div className="flex items-start gap-3"><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${contact.id === contactId ? 'bg-[#E85D52] text-white' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}>{contact.id === contactId ? <Check className="h-4 w-4" /> : <UsersRound className="h-4 w-4" />}</span><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-950 dark:text-white">{contact.name}</p><p className="mt-0.5 truncate text-xs text-slate-500">{contact.contact_person || contact.phone || contact.email || 'Sin datos adicionales'}</p>{contact.fiscal_tax_id ? <p className="mt-1 truncate text-[11px] font-medium text-[#C94840]">{contact.fiscal_country === 'MX' ? 'RFC' : 'ID fiscal'}: {contact.fiscal_tax_id}</p> : null}</div></div>
                  </button>
                ))}
          ...
          {!createdSale && step === 2 ? (
            <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
              <div><h3 className="font-semibold text-slate-950 dark:text-white">Productos de la venta</h3><p className="mt-1 text-xs text-slate-500">Precio e impuesto vienen del catálogo; tú capturas la cantidad.</p></div>
                ...
                {visibleProducts.map(product => {
                  ...
                  return (
                    <article key={product.id} className={`rounded-xl border p-3 ${quantity ? 'border-rose-300 bg-rose-50/50 dark:border-rose-900 dark:bg-rose-950/20' : 'border-slate-200 dark:border-slate-700'}`}>
                      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-950 dark:text-white">{product.name}</p><p className="truncate text-xs text-slate-500">{product.sku || product.code || product.category}</p></div><p className="shrink-0 text-sm font-bold text-slate-950 dark:text-white">{money(product.price, product.currency, locale)}</p></div>
                      <div className="mt-2 flex items-center justify-between gap-3"><p className={`text-xs ${warehouseId && !isService && available <= 0 ? 'text-red-600' : 'text-slate-500'}`}>{!warehouseId ? 'Selecciona un almacén' : isService ? 'Servicio' : `${available} disponibles`} · Imp. {product.tax_percent}%</p><div className="flex items-center rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-950"><button type="button" aria-label={`Quitar ${product.name}`} className="grid h-11 w-11 place-items-center disabled:opacity-30" disabled={!warehouseId || quantity <= 0} onClick={() => setQuantity(product, quantity - 1)}><Minus className="h-4 w-4" /></button><span className="min-w-8 text-center text-sm font-semibold">{quantity}</span><button type="button" aria-label={`Agregar ${product.name}`} className="grid h-11 w-11 place-items-center text-[#C94840] disabled:opacity-30" disabled={!warehouseId || incompatibleCurrency || (!isService && quantity >= available)} onClick={() => setQuantity(product, quantity + 1)}><Plus className="h-4 w-4" /></button></div></div>
                    </article>
                  );
                })}
              </div>
              {cartLines.length ? <div className="flex items-center justify-between rounded-xl bg-slate-950 px-4 py-3 text-white dark:bg-white dark:text-slate-950"><span className="text-sm">{cartLines.reduce((sum, line) => sum + line.quantity, 0)} artículos</span><strong>{money(total, currency, locale)}</strong></div> : null}
            </section>
          ) : null}
          ...
          {!createdSale && step === 3 ? (
            <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
              <div><h3 className="font-semibold text-slate-950 dark:text-white">¿Cómo paga el cliente?</h3><p className="mt-1 text-xs text-slate-500">La venta termina aquí; Finanzas concilia el cobro después.</p></div>
              ...
              {paymentMethod !== 'credit' ? <div className="rounded-2xl border border-slate-200 p-3 dark:border-slate-700">
                <div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-rose-50 text-[#C94840] dark:bg-rose-950/30 dark:text-rose-200"><FileText className="h-5 w-5" /></span><div><p className="text-sm font-semibold text-slate-950 dark:text-white">Comprobante del cobro</p><p className="mt-0.5 text-xs text-slate-500">Opcional · foto o PDF · máximo 15 MB.</p></div></div>
                {canAttachEvidence ? <div className="mt-3 grid grid-cols-2 gap-2">
                  <label className="inline-flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#E85D52] px-3 text-sm font-semibold text-white outline-none focus-within:ring-4 focus-within:ring-rose-200"><Camera className="h-4 w-4" />Tomar foto<input type="file" className="sr-only" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={selectEvidence} /></label>
                  <label className="inline-flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-semibold text-slate-700 outline-none focus-within:ring-4 focus-within:ring-rose-100 dark:border-slate-700 dark:text-slate-200"><Upload className="h-4 w-4" />Elegir archivo<input type="file" className="sr-only" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={selectEvidence} /></label>
          ...
          {!createdSale && step === 4 ? (
            <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
              <div><h3 className="font-semibold text-slate-950 dark:text-white">Revisa y termina la venta</h3><p className="mt-1 text-xs text-slate-500">Al confirmar se registra la venta y se descuenta el inventario.</p></div>
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-[11px] uppercase tracking-wide text-slate-500">Cliente</p><p className="mt-1 text-sm font-semibold text-slate-950 dark:text-white">{selectedContact?.name}</p></div><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-[11px] uppercase tracking-wide text-slate-500">Almacén</p><p className="mt-1 text-sm font-semibold text-slate-950 dark:text-white">{warehouses.find(item => item.id === warehouseId)?.name}</p></div><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-[11px] uppercase tracking-wide text-slate-500">Cobro</p><p className="mt-1 text-sm font-semibold capitalize text-slate-950 dark:text-white">{paymentMethods.find(item => item.id === paymentMethod)?.label || paymentMethod}</p></div><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-[11px] uppercase tracking-wide text-slate-500">Comprobante</p><p className="mt-1 truncate text-sm font-semibold text-slate-950 dark:text-white">{paymentMethod === 'credit' ? 'No aplica' : evidenceFile?.name || 'Sin archivo'}</p></div></div>
              <div className="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-slate-800 dark:border-slate-700">{cartLines.map(line => <div key={line.product.id} className="flex items-center justify-between gap-3 p-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-950 dark:text-white">{line.product.name}</p><p className="text-xs text-slate-500">{line.quantity} × {money(line.product.price, line.product.currency, locale)} · Imp. {line.product.tax_percent}%</p></div><strong className="text-sm text-slate-950 dark:text-white">{money(line.total, line.product.currency, locale)}</strong></div>)}</div>
              <div className="space-y-1 rounded-xl bg-rose-50 p-4 dark:bg-rose-950/25"><div className="flex justify-between text-sm text-slate-600 dark:text-slate-300"><span>Subtotal</span><span>{money(subtotal, currency, locale)}</span></div><div className="flex justify-between text-sm text-slate-600 dark:text-slate-300"><span>Impuestos</span><span>{money(taxTotal, currency, locale)}</span></div><div className="flex justify-between border-t border-rose-200 pt-2 text-lg font-bold text-slate-950 dark:border-rose-900 dark:text-white"><span>Total</span><span>{money(total, currency, locale)}</span></div></div>
      ...
      {tab === 'sales' ? (
        <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
          <div><h3 className="font-semibold text-slate-950 dark:text-white">Mis ventas</h3><p className="mt-1 text-xs text-slate-500">Seguimiento de las ventas registradas con tu usuario.</p></div>
          <div className="grid gap-2">{recentSales.map(sale => <article key={sale.id} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-950 dark:text-white">{sale.customer_name}</p><p className="mt-0.5 text-xs text-slate-500">{sale.sale_number} · {dateLabel(sale.sale_date, locale)}</p></div><strong className="shrink-0 text-sm text-slate-950 dark:text-white">{money(sale.total_amount, sale.currency, locale)}</strong></div><div className="mt-3 flex flex-wrap gap-1.5"><span className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${statusClass(sale.inventory_status)}`}>Inventario: {statusLabel(sale.inventory_status)}</span><span className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${statusClass(sale.delivery_status)}`}>Entrega: {statusLabel(sale.delivery_status)}</span><span className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${statusClass(sale.finance_status)}`}>Cobro: {statusLabel(sale.finance_status)}</span><span className={`rounded-full border px-2 py-1 text-[10px] font-semibold ${statusClass(sale.payment_evidence_status || 'missing')}`}>Comprobante: {sale.payment_evidence_status === 'not_required' ? 'No aplica' : Number(sale.evidence_count || 0) > 0 ? 'Enviado' : 'Pendiente'}</span></div></article>)}</div>
      ...
      {tab === 'customers' ? (
        <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950 sm:p-5">
          <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-slate-950 dark:text-white">Mis clientes</h3><p className="mt-1 text-xs text-slate-500">Tu cartera disponible para ventas en ruta.</p></div><Button type="button" size="sm" className="min-h-11 bg-[#E85D52] text-white hover:bg-[#cf4d44]" onClick={() => setShowNewContact(true)}><UserPlus className="mr-1.5 h-4 w-4" />Nuevo</Button></div>
          {showNewContact ? contactForm : null}
          ...
          <div className="grid gap-2 sm:grid-cols-2">{visibleContacts.map(contact => <article key={contact.id} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><div className="flex gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-rose-50 text-[#C94840] dark:bg-rose-950/30 dark:text-rose-300"><UsersRound className="h-4 w-4" /></span><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-950 dark:text-white">{contact.name}</p><p className="mt-0.5 truncate text-xs text-slate-500">{contact.contact_person || contact.code || 'Cliente de ruta'}</p><p className="mt-1 truncate text-xs text-slate-500">{contact.phone || contact.email || 'Sin contacto capturado'}</p>{contact.fiscal_tax_id ? <p className="mt-1 truncate text-[11px] font-semibold text-[#C94840]">{contact.fiscal_country === 'MX' ? 'RFC' : 'ID fiscal'}: {contact.fiscal_tax_id}</p> : <p className="mt-1 text-[11px] text-slate-400">Sin datos fiscales</p>}</div></div><Button type="button" size="sm" variant="outline" className="mt-3 min-h-11 w-full" onClick={() => { setContactId(contact.id); setTab('sell'); setStep(2); }}>Vender a este cliente</Button></article>)}</div>
        </section>
      ) : null}
    </div>
  );
