  ...
  return (
    ...
    >
      ...
      <div className="mt-4 space-y-2 text-sm">
        ...
        <p className="text-slate-500">{formatCurrencyAmount(parseMoney(opportunity.estimatedValue))} · {opportunity.probability}</p>
      </div>

      <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
        <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Siguiente acción</p>
        <p className="mt-1 text-sm font-semibold text-slate-900">{opportunity.nextAction}</p>
        <p className="mt-1 text-xs font-semibold text-[#9a6b05]">{opportunity.nextActionDate || 'Sin fecha'}</p>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <p className="text-xs font-semibold text-slate-400">Arrastra para cambiar etapa</p>
