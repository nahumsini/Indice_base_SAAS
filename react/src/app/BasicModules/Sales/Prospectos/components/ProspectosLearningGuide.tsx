...
const guideIcons = [Target, CalendarCheck2, FileText] as const;

export function ProspectosLearningGuide({ copy }: ProspectosLearningGuideProps) {
  const [isCollapsed, setIsCollapsed] = useLocalStorageState<boolean>(
    'indice.sales.prospectos.learningGuide.collapsed',
    false,
  );
    ...
    <section className="overflow-hidden rounded-lg border border-[#FF6B5E]/25 bg-white shadow-sm dark:border-[#FFB4AD]/20 dark:bg-slate-950">
      <div className="border-b border-[#FF6B5E]/10 bg-[#FF6B5E]/[0.06] p-5 dark:border-[#FFB4AD]/10 dark:bg-[#FF6B5E]/10">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#FF6B5E]/25 bg-white px-3 py-1 text-[11px] font-medium tracking-normal text-[#B63B32] shadow-sm dark:border-[#FFB4AD]/25 dark:bg-slate-950 dark:text-[#FFD8D4]">
              <Lightbulb className="h-3.5 w-3.5" />
              {copy.eyebrow}
            </div>
            <h3 className="mt-3 text-xl font-medium text-slate-950 dark:text-white">{copy.title}</h3>
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-[#FF6B5E]/25 bg-white px-4 text-sm font-medium text-[#B63B32] shadow-sm transition hover:bg-[#FF6B5E]/10 dark:border-[#FFB4AD]/25 dark:bg-slate-950 dark:text-[#FFD8D4] dark:hover:bg-slate-900"
          >
      ...
      {isCollapsed ? null : (
        <div className="space-y-4 p-5">
          <div>
            <p className="text-xs font-medium tracking-normal text-[#B63B32] dark:text-[#FFD8D4]">
              {copy.flowTitle}
            </p>
                ...
                {copy.cards.map((card, index) => {
                  ...
                  return (
                    <article key={card.title} className="rounded-lg border border-white/80 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/75">
                      ...
                      <p className="mt-3 text-sm font-medium text-slate-950 dark:text-white">{card.title}</p>
              ...
              <div className="mt-3 space-y-2">
                {copy.checklist.map((item) => (
                  <div key={item} className="flex items-start gap-2 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <p className="text-xs font-medium leading-5 text-slate-700 dark:text-slate-200">{item}</p>
                  </div>
                ))}
              </div>
              <p className="mt-3 rounded-lg border border-[#FF6B5E]/15 bg-[#FF6B5E]/[0.05] px-3 py-2 text-xs font-medium leading-5 text-[#B63B32] dark:border-[#FFB4AD]/15 dark:bg-[#FFB4AD]/10 dark:text-[#FFD8D4]">
                {copy.footer}
              </p>
            </aside>
          </div>
