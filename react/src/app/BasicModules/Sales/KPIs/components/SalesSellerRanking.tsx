          ...
          <tbody>
            {rows.map((row, index) => (
              <tr key={row.seller} className="border-t border-slate-100 dark:border-slate-800">
                <td className="px-5 py-4 font-medium text-slate-950 dark:text-white">#{index + 1}</td>
                <td className="px-5 py-4 font-medium text-slate-950 dark:text-white">{row.seller}</td>
                <td className="px-5 py-4 font-medium text-slate-900 dark:text-slate-100">{sellerMoney.get(row.seller)?.sales}</td>
                ...
                <td className="px-5 py-4 font-medium text-slate-900 dark:text-white">{percent(row.conversion)}</td>
              </tr>
            ))}
          </tbody>
        </table>
