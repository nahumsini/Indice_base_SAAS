...
function DetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-w-0 items-start justify-between gap-4 py-2 first:pt-0 last:pb-0">
      ...
      <dd className="min-w-0 max-w-[65%] break-words text-right text-sm font-medium leading-5 text-slate-950 dark:text-white">
        {value}
      </dd>
    </div>
  );
...
function getStatusClassName(status: string) {
  ...
  if (['reserved', 'uploaded', 'in_progress'].includes(status)) {
    return 'border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-200';
  }
  if (['rejected', 'cancelled', 'unavailable'].includes(status)) {
...
function StatusSummaryItem({ label, status, value }: { label: string; status: string; value: string }) {
  return (
    <div className="flex min-w-0 items-center justify-between gap-3 px-4 py-3 sm:block sm:px-5">
      <p className="text-xs font-normal text-slate-500 dark:text-slate-400">{label}</p>
      <span className={cn('inline-flex max-w-full rounded-full border px-2.5 py-1 text-xs font-medium sm:mt-2', getStatusClassName(status))}>
  ...
  return (
    <Collapsible>
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900/50">
        <CollapsibleTrigger className="flex min-h-14 w-full items-start gap-3 px-4 py-3 text-left transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B5E]/45 focus-visible:ring-inset dark:hover:bg-slate-800/50 sm:px-5 [&[data-state=open]>svg]:rotate-180">
        ...
        <CollapsibleContent className="border-t border-slate-100 px-4 py-3.5 dark:border-slate-800 sm:px-5">
          {children}
        </CollapsibleContent>
      </section>
    </Collapsible>
...
export function SalesRecordDetailView({
  ...
  onPreviewSalesNote: () => void;
  onStatusChange: (patch: Partial<SaleRecord>) => void;
}) {
  const notAvailable = t.common.notAvailable;
  const documentReference = form.saleNumber || record.saleNumber || form.saleDocumentReference || notAvailable;
  const paymentMethod = normalizeSalesPaymentMethod(form.paymentMethod);
  const paymentMethodLabel = paymentMethod ? t.modal.paymentMethods[paymentMethod] : form.paymentMethod || notAvailable;
  const movementReference = !form.inventoryMovementReference || /(^|[-_])DRAFT($|[-_])/i.test(form.inventoryMovementReference)
    ? t.modal.detail.movementNotGenerated
    : form.inventoryMovementReference;

  return (
    <div className="space-y-3.5">
      <section className="overflow-hidden rounded-2xl border border-rose-200 bg-white dark:border-rose-900/60 dark:bg-slate-900/50">
        <div className="flex flex-col gap-3 bg-gradient-to-br from-rose-50 via-white to-orange-50 px-4 py-3.5 dark:from-rose-950/30 dark:via-slate-900 dark:to-orange-950/20 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div className="min-w-0">
            ...
            <h3 className="mt-0.5 truncate text-lg font-medium text-slate-950 dark:text-white" title={documentReference}>{documentReference}</h3>
            <p className="mt-0.5 truncate text-sm font-normal text-slate-600 dark:text-slate-300" title={form.customerName || notAvailable}>
          ...
          <div className="flex shrink-0 flex-col items-stretch gap-3 sm:items-end">
            <div className="sm:text-right">
              ...
              <p className="mt-0.5 text-2xl font-medium tracking-tight text-slate-950 dark:text-white">
                {formatSalesCurrency(form.totalAmount, form.currency)}
              </p>
            </div>
            <Button type="button" variant="outline" className={cn('h-9 w-full gap-2 bg-white px-3 text-sm font-medium sm:w-auto', actionClassNames.secondary)} onClick={onPreviewSalesNote} disabled={!form.customerName.trim()}>
        ...
        <div className="grid divide-y divide-slate-100 dark:divide-slate-800 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
          <SummaryItem icon={<CalendarDays className="h-4 w-4" />} label={t.modal.fields.saleDate} value={formatSalesDate(form.saleDate, locale)} />
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900/50">
        <header className="border-b border-slate-100 px-4 py-3 dark:border-slate-800 sm:px-5">
          <h3 className="text-sm font-medium text-slate-950 dark:text-white">{t.modal.sections.operationalProgress}</h3>
          <p className="mt-0.5 text-xs font-normal leading-5 text-slate-500 dark:text-slate-400">{t.modal.detail.progressHelper}</p>
        </header>
        <div className="grid divide-y divide-slate-100 dark:divide-slate-800 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
          <StatusSummaryItem label={t.modal.fields.paymentEvidenceStatus} status={form.paymentEvidenceStatus} value={t.statuses.paymentEvidence[form.paymentEvidenceStatus]} />
          <StatusSummaryItem label={t.modal.fields.financeStatus} status={form.financeStatus} value={t.statuses.finance[form.financeStatus]} />
          <StatusSummaryItem label={t.modal.fields.inventoryStatus} status={form.inventoryStatus} value={t.statuses.inventory[form.inventoryStatus]} />
          <StatusSummaryItem label={t.modal.fields.inventoryMovementStatus} status={form.inventoryMovementStatus} value={t.statuses.movement[form.inventoryMovementStatus]} />
        </div>
      </section>
        ...
        <div className="grid divide-y divide-slate-100 dark:divide-slate-800 md:grid-cols-2 md:divide-x md:divide-y-0">
          <div className="p-4 sm:p-5">
            <div className="mb-4 flex items-center gap-3">
              <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"><CircleDollarSign className="h-4 w-4" /></span>
              <h3 className="text-sm font-medium text-slate-950 dark:text-white">{t.modal.sections.payment}</h3>
            </div>
          ...
          <div className="p-4 sm:p-5">
            <div className="mb-4 flex items-center gap-3">
              <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"><PackageCheck className="h-4 w-4" /></span>
              <h3 className="text-sm font-medium text-slate-950 dark:text-white">{t.modal.sections.inventory}</h3>
            </div>
            <dl className="divide-y divide-slate-100 dark:divide-slate-800">
              <DetailItem label={t.modal.fields.warehouse} value={form.warehouseName || operationalContext.defaultWarehouse || notAvailable} />
              <DetailItem label={t.modal.fields.inventoryMovementReference} value={movementReference} />
            </dl>
          </div>
        </div>
      </section>
      ...
      <CollapsibleDetailSection icon={<Building2 className="h-4 w-4" />} title={t.modal.sections.operationalContext} description={t.modal.operationalContext.helper}>
        <div className="grid gap-x-8 md:grid-cols-2">
          <dl className="divide-y divide-slate-100 dark:divide-slate-800">
            ...
            <DetailItem label={t.modal.operationalContext.legalName} value={operationalContext.legalName || notAvailable} />
          </dl>
          <dl className="divide-y divide-slate-100 dark:divide-slate-800">
            <DetailItem label={t.modal.operationalContext.taxIdentifier} value={operationalContext.taxIdentifier || notAvailable} />
            <DetailItem label={t.modal.operationalContext.fiscalAddress} value={operationalContext.fiscalAddress || notAvailable} />
            <DetailItem label={t.modal.fields.currency} value={operationalContext.currency || notAvailable} />
          </dl>
        </div>
      </CollapsibleDetailSection>
            ...
            {commissionRecords.map((commission) => (
              <div key={commission.id} className="grid gap-2 py-3 first:pt-0 last:pb-0 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-xs font-normal text-slate-500 dark:text-slate-400">{commission.salesRepName} · {commission.commissionRuleName} · {formatCommissionType(commission.commissionType)}</p>
                </div>
                <div className="sm:text-right">
      ...
      <CollapsibleDetailSection icon={<ClipboardCheck className="h-4 w-4" />} title={t.modal.sections.notes}>
