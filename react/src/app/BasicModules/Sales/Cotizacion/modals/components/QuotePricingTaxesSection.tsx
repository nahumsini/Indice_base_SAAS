    ...
    <section className="space-y-4">
      <div className="rounded-lg border border-[#FF6B5E]/20 bg-[#FF6B5E]/5 p-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-1 text-sm font-normal leading-6 text-slate-600">{t.pricing.taxHelper}</p>
          </div>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-sm font-medium text-slate-500">{t.labels.subtotal}</p>
          <p className="mt-1 text-xl font-semibold text-slate-950">{formatCurrency(totals.subtotal, form.currency)}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-sm font-medium text-slate-500">{t.labels.discountTotal}</p>
          <p className="mt-1 text-xl font-semibold text-slate-950">{formatCurrency(totals.discountTotal, form.currency)}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-sm font-medium text-slate-500">{t.labels.taxTotal}</p>
          <p className="mt-1 text-xl font-semibold text-slate-950">{formatCurrency(totals.taxTotal, form.currency)}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-sm font-medium text-slate-500">{t.pricing.estimatedCost}</p>
          <p className="mt-1 text-xl font-semibold text-slate-950">{formatCurrency(totals.estimatedCost, form.currency)}</p>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-sm font-medium text-slate-500">{t.pricing.estimatedProfit}</p>
          <p className="mt-1 text-xl font-semibold text-slate-950">{formatCurrency(totals.estimatedProfit, form.currency)}</p>
        </div>
        <div className="rounded-lg border border-[#FF6B5E]/25 bg-[#FF6B5E]/10 p-4">
          ...
          <p className="mt-1 text-xl font-bold text-[#B63B32]">{formatCurrency(totals.total, form.currency)}</p>
        </div>
      </div>
