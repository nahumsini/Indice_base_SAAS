...
export function MovementsKanban({
  ...
  return (
    <>
      ...
      {dragState ? (
        <div
          className="pointer-events-none fixed z-[9999] rounded-xl border border-[#FF6B5E]/30 bg-white px-3 py-2 text-xs font-black text-slate-900 shadow-2xl ring-4 ring-[#FF6B5E]/10"
          style={{ left: dragState.x + 12, top: dragState.y + 12 }}
        >
          {dragState.label}
          <span className={`ml-2 rounded-md border px-2 py-0.5 uppercase ${movementStatusTone[dragState.status]}`}>
      ...
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-black text-slate-900">{t.operational.movementStatuses[status]}</h3>
          </div>
