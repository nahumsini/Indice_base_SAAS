      ...
      <div className="overflow-x-auto">
        <Table className="min-w-[1280px] table-fixed">
          ...
          <TableBody>
            {locations.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} className="px-5 py-12 text-center text-sm font-semibold text-slate-500 dark:text-slate-300">{t.locationsView.empty}</TableCell>
              </TableRow>
              ...
              return (
                <TableRow key={location.id} className="border-slate-200 align-top hover:bg-slate-50/80 dark:border-slate-700 dark:hover:bg-slate-700/40">
                  <TableCell className="px-5 py-5">
                    <p className="font-black text-slate-950 dark:text-white">{location.name}</p>
                  ...
                  <TableCell className="px-5 py-5 font-semibold text-slate-700 dark:text-slate-300">{t.locationTypes[location.type]}</TableCell>
                  <TableCell className="px-5 py-5 font-semibold text-slate-700 dark:text-slate-300">{t.scopeTypes[location.scopeType]}</TableCell>
                  <TableCell className="px-5 py-5 font-semibold text-slate-700 dark:text-slate-300">{linkedLabel(location, t)}</TableCell>
                  <TableCell className="px-5 py-5 font-black text-slate-950 dark:text-white">{summary.trackedSkus}</TableCell>
                  <TableCell className="px-5 py-5 font-black text-slate-950 dark:text-white">{formatInventoryCurrency(summary.estimatedValue)}</TableCell>
                  <TableCell className="px-5 py-5">
                  ...
                  <TableCell className="px-5 py-5 font-semibold text-slate-700 dark:text-slate-300">{location.managerName ?? t.common.notAvailable}</TableCell>
                </TableRow>
              );
            })}
          </TableBody>
