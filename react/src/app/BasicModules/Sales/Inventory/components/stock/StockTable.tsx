...
const numberClass = 'text-right font-medium tabular-nums text-slate-900 dark:text-white';
...
function ProductPhoto({ row, t }: { row: InventoryStockRow; t: InventoryTranslations }) {
  ...
  return (
    <span className="flex h-12 w-12 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-[10px] font-medium text-slate-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-500">
            ...
            {paginatedRows.map((row) => {
              ...
              return (
                <TableRow key={row.id} className={`border-slate-100 dark:border-slate-700 ${selected ? 'bg-[#FF6B5E]/5 dark:bg-[#FF6B5E]/10' : 'hover:bg-slate-50/80 dark:hover:bg-slate-700/40'}`}>
                  ...
                  {canShow('product') ? <TableCell className="min-w-[260px] px-3 py-2"><div className="text-sm font-medium leading-5 text-slate-950 dark:text-white">{row.name}</div><div className="mt-1 flex flex-wrap items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-300"><span className="rounded bg-slate-100 px-1.5 py-0.5 font-medium text-slate-600 dark:bg-slate-900 dark:text-slate-300">{row.sku}</span><span className="max-w-[240px] truncate">{row.description}</span></div></TableCell> : null}
                  {canShow('sku') ? <TableCell className="px-3 py-2 font-medium text-slate-600 dark:text-slate-300">{row.sku}</TableCell> : null}
                  {canShow('category') ? (
                    <TableCell className="px-3 py-2">
                      <Select value={row.category} onValueChange={(category) => onCategoryChange(row, category as InventoryStockRow['category'])}>
                        <SelectTrigger className="h-9 min-w-[150px] rounded-full border-slate-200 bg-white px-3 text-xs font-medium text-slate-800 shadow-none focus:ring-[#FF6B5E]/20 dark:border-slate-700 dark:bg-slate-900 dark:text-white">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  {canShow('type') ? <TableCell className="px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-300">{row.type}</TableCell> : null}
                  ...
                  {canShow('status') ? <TableCell className="px-3 py-2"><span className={`rounded-full border px-3 py-1 text-xs font-medium ${statusTone[health]}`}>{t.operational.statuses[health]}</span></TableCell> : null}
                  ...
                  {canShow('lastMovement') ? <TableCell className="px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-300">{row.lastMovementAt ?? t.common.notAvailable}</TableCell> : null}
                  <TableCell className="px-3 py-2">
