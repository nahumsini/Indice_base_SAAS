...
export function StockWarehouseDistribution({
  ...
  if (distributions.length === 0) {
    return <span className="text-xs font-medium text-slate-400">{t.common.notAvailable}</span>;
  }
      ...
      {distributions.map((distribution) => {
        ...
        return (
          <span
            ...
            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium leading-none before:h-1.5 before:w-1.5 before:rounded-full ${toneByHealth[health]}`}
