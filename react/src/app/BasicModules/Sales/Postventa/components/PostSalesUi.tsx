  ...
  return (
    <span className="inline-flex items-center gap-3">
      ...
      <span className="text-base font-medium">
        <span className={cn('mr-2 font-medium dark:text-white', valueClassName)}>{value}</span>
  ...
  return (
    <div className="space-y-2">
      ...
      <Select value={value} onValueChange={onValueChange}>
        <SelectTrigger className="h-11 rounded-lg border-slate-200 bg-white px-4 text-base font-medium text-slate-950 shadow-none dark:border-slate-700 dark:bg-slate-900 dark:text-white">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {options.map((option) => (
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="mt-4 space-y-3">
        {cases.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-sm font-medium text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
            {emptyLabel}
          </div>
          ...
          <article key={postSaleCase.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-900">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate font-medium text-slate-950 dark:text-white">{postSaleCase.clientName}</p>
                <p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-300">{postSaleCase.nextAction}</p>
              </div>
              <Badge className={cn('rounded-full border px-2 py-1 text-xs font-medium', riskClasses[postSaleCase.riskLevel])}>
