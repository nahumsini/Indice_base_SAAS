  ...
  return (
    ...
    >
      ...
      {visibleColumns.includes('sku') ? (
        <TableCell className="truncate px-5 py-4 align-middle font-semibold text-slate-700">{product.sku}</TableCell>
      ) : null}
      ...
      {visibleColumns.includes('type') ? (
        <TableCell className="truncate px-5 py-4 align-middle font-semibold text-slate-700">{t.typeLabels[product.type]}</TableCell>
      ) : null}
      {visibleColumns.includes('price') ? (
        <TableCell className="truncate px-5 py-4 align-middle font-black text-slate-950">{formatProductCurrency(product.price)}</TableCell>
      ) : null}
      {visibleColumns.includes('cost') ? (
        <TableCell className="truncate px-5 py-4 align-middle font-semibold text-slate-600">{formatProductCurrency(product.cost)}</TableCell>
      ) : null}
      {visibleColumns.includes('profit') ? (
        <TableCell className="px-5 py-4 align-middle">
          <p className="font-black text-[#177d66]">{formatProductCurrency(getProductProfit(product))}</p>
      ...
      {visibleColumns.includes('lastUpdated') ? (
        <TableCell className="truncate px-5 py-4 align-middle font-semibold text-slate-600">{product.lastUpdated}</TableCell>
      ) : null}
      <TableCell className="px-5 py-4 align-middle">
