...
export function getProductAvailability(product: SalesCatalogItem): ProductAvailabilityKey[] {
  ...
  const isReadyForSales = product.status === 'Active' && !isInternal && product.price > 0;

  if (isReadyForSales) {
    availability.push('sales');
  }

  if (product.posPrepared || product.visibility === 'POS ready') {
    availability.push('pos');
  }

  if (product.stockPrepared || product.warehousePrepared) {
    availability.push('inventory');
  }

  if (isInternal || availability.length === 0) {
    availability.push('internal');
  }
...
export function getProductHealthWarnings(product: SalesCatalogItem): ProductHealthKey[] {
  ...
  if (!product.category) {
    warnings.push('missingCategory');
  }

  if (!hasImage) {
    warnings.push('missingImage');
  }

  if (product.price <= 0) {
    warnings.push('missingPrice');
  }

  if (!availability.includes('sales') && product.type !== 'Operational item') {
    warnings.push('notReadyForSales');
  }

  if (product.price > 0 && margin < 20) {
    warnings.push('lowMargin');
  }

  if (warnings.length > 0) {
    warnings.push('requiresReview');
  }
