  ...
  return (
    <header className="bg-white dark:bg-slate-950">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-8 md:px-8 lg:flex-row lg:items-center lg:justify-between">
        ...
        {href ? (
          <Button asChild className="h-11 gap-2 rounded-lg bg-[#FF6B5E] px-5 text-sm font-black text-white hover:bg-[#E85C50]">
      ...
      {config.coverImageUrl ? (
        <div className="mx-auto max-w-7xl px-5 pb-6 md:px-8">
          <img
            ...
            alt={config.title}
            className="h-56 w-full rounded-lg border border-slate-200 object-cover shadow-sm dark:border-slate-800"
          />
        </div>
      ) : null}
    </header>
