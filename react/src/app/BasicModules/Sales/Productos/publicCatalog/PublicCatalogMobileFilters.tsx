...
export function PublicCatalogMobileFilters({
  ...
  const chipClassName = (active: boolean) => (
    `h-9 shrink-0 rounded-full border px-3 text-xs font-bold transition ${active
      ? 'border-[#FF6B5E] bg-[#FF6B5E] text-white shadow-sm'
  ...
  return (
    <section className="sticky top-0 z-20 w-full min-w-0 max-w-full overflow-hidden border-b border-slate-200 bg-slate-50/95 px-4 pb-3 pt-3 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        ...
        {types.length > 1 ? (
          <select
            ...
            className="h-9 max-w-[170px] rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold text-slate-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
          >
