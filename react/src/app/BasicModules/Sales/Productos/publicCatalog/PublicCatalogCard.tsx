  ...
  return (
    <article className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <div className="space-y-3 p-4">
        <div>
          <h3 className="line-clamp-2 text-lg font-black text-slate-950">{item.name}</h3>
          {config.showCategories && item.category ? (
            <p className="mt-1 text-sm font-semibold text-slate-500">{t.categoryLabels[item.category]}</p>
          ) : null}
        </div>
        ...
        <Button type="button" className="w-full gap-2 rounded-lg bg-[#FF6B5E] text-white hover:bg-[#E85C50]" onClick={() => onAddToCart(item)}>
