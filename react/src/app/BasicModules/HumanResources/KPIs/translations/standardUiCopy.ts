const copies = {
  ...
  zh: {
    allStatuses: '所有状态', clear: '清除', results: (count: number) => `${count} 条结果`, operationalDate: '运营日期', status: '考勤状态', comparison: '比较：等效的上一日期或期间', topUnits: '需要关注的单位', topDepartments: '存在异常的部门', topAttention: '需要关注的员工', performanceTitle: '按员工进行运营跟踪', performanceSubtitle: '评分：考勤45%、请假20%、记录25%、已分配资产10%。', focus: '聚焦', approved: '已批准', rejected: '已拒绝', reviewed: '审核中', resolved: '已解决', pagination: { next: '下一页', previous: '上一页', rows: '每页行数', item: '员工' },
  },
} as const;
