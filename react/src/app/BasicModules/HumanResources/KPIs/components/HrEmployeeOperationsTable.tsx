  ...
  return (
    <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-5 dark:border-slate-700 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span className="inline-flex w-fit items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300">
          <Users className="h-4 w-4" /> {rows.length}
        </span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1100px] text-left">
          <thead className="bg-slate-50 text-xs font-bold uppercase tracking-[0.14em] text-slate-500 dark:bg-slate-900/60 dark:text-slate-400">
            <tr>
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {pagination.paginatedRows.map(row => (
              <tr key={row.id} className="hover:bg-emerald-50/30 dark:hover:bg-emerald-950/10">
                <td className="px-5 py-4 text-lg font-black text-slate-800 dark:text-white">#{row.position}</td>
                <td className="px-5 py-4"><p className="font-bold text-slate-900 dark:text-white">{row.name}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{row.meta}</p></td>
                <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="h-2 w-24 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700"><div className={`h-full rounded-full ${barClasses[row.status]}`} style={{ width: `${row.score}%` }} /></div><span className="font-black text-slate-900 dark:text-white">{row.score}</span><span className={`rounded-full border px-2 py-1 text-[11px] font-bold ${badgeClasses[row.status]}`}>{labels.statuses[row.status]}</span></div></td>
                <td className="px-5 py-4 text-sm font-semibold text-slate-700 dark:text-slate-200">{row.attendance}</td>
                <td className="px-5 py-4 text-sm font-bold text-amber-600 dark:text-amber-300">{row.permissions}</td>
                <td className="px-5 py-4 text-sm font-bold text-rose-600 dark:text-rose-300">{row.records}</td>
                <td className="px-5 py-4 text-sm font-semibold text-slate-700 dark:text-slate-200">{row.assets}</td>
                <td className="px-5 py-4"><button type="button" onClick={() => onFocus(row)} aria-label={`${labels.focus}: ${row.name}`} className="inline-flex h-10 items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-sm font-bold text-emerald-700 transition hover:bg-emerald-100 focus:outline-none focus:ring-2 focus:ring-emerald-300 dark:border-emerald-900/50 dark:bg-emerald-950/20 dark:text-emerald-300"><Search className="h-4 w-4" />{labels.focus}</button></td>
              </tr>
            ))}
            {pagination.totalCount === 0 ? <tr><td colSpan={8} className="px-5 py-12 text-center text-sm font-semibold text-slate-500 dark:text-slate-400">{labels.noRows}</td></tr> : null}
          </tbody>
        </table>
      </div>
      <DataTablePagination
