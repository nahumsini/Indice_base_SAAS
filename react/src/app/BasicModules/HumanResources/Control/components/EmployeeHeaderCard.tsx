    ...
    <div className="rounded-lg border border-[#59C3A5]/10 bg-[#fbfdff] p-4 shadow-sm dark:border-gray-800 dark:bg-gray-800">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="truncate text-xl font-medium text-gray-900 dark:text-white">{selectedEmployee.user_name}</p>
          <p className="mt-1 text-sm font-medium text-gray-500 dark:text-gray-400">{role}</p>
        </div>
        <span className={`inline-flex w-fit rounded-full px-3 py-1.5 text-sm font-medium ${primaryStatus.className}`}>
          {primaryStatus.label}
        </span>
      </div>
