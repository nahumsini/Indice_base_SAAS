...
export async function loadPettyCashKpiAggregates(queries: KpiMonetaryBatchQuery[], isActive = () => true) {
  ...
  for (let offset = 0; offset < queries.length && isActive(); offset += 100) {
    ...
    if (batch.some(query => !response[query.key])) throw new Error('Incomplete petty cash KPI response');
    Object.assign(results, response);
  }
  return results;
}

export function usePettyCashKpiAggregates(queries: KpiMonetaryBatchQuery[], sourceRevision: object, enabled = true) {
  ...
  useEffect(() => {
    if (!enabled) return;
    let active = true;
    loadPettyCashKpiAggregates(JSON.parse(key), () => active)
      .then(data => { if (active) setSnapshot({ key, revision: sourceRevision, data, error: null }); })
      .catch(error => { if (active) setSnapshot({ key, revision: sourceRevision, data: {}, error: error instanceof Error ? error : new Error('Petty cash KPI request failed') }); });
    return () => { active = false; };
