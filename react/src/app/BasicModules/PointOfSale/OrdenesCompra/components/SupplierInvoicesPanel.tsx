  ...
  return (
    <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-lg font-medium text-slate-950 dark:text-white">Facturas por conciliar</h3>
          <p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-400">
            Valida que la factura corresponda a la compra recibida antes de mandarla a pago.
          </p>
        </div>
        <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-medium text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
        ...
        {invoices.length === 0 ? (
          ...
          <article key={invoice.id} className="rounded-[18px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              {invoice.documentUrl ? (
                <a href={invoice.documentUrl} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">
                  <ExternalLink className="h-4 w-4" />
                  Ver documento
                </a>
              ) : null}
              {invoice.status === 'SUBMITTED' && (
                <button type="button" disabled={disabled} onClick={() => onReview(invoice.id, 'MATCHED')} className="inline-flex h-9 items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 text-xs font-medium text-blue-700 disabled:opacity-60 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-200">
              ...
              {(invoice.status === 'SUBMITTED' || invoice.status === 'MATCHED') && (
                <>
                  <button type="button" disabled={disabled} onClick={() => onReview(invoice.id, 'APPROVED_FOR_PAYMENT')} className="inline-flex h-9 items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-xs font-medium text-emerald-700 disabled:opacity-60 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-200">
                    <ShieldCheck className="h-4 w-4" />
                    Aprobar pago
                  </button>
                  <button type="button" disabled={disabled} onClick={() => onReview(invoice.id, 'REJECTED')} className="inline-flex h-9 items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-3 text-xs font-medium text-red-700 disabled:opacity-60 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-200">
