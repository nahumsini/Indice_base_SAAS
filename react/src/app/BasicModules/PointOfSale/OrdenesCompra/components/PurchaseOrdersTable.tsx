  ...
  if (orders.length === 0) {
    return (
      <section className="rounded-[24px] border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <p className="text-lg font-semibold text-slate-950 dark:text-white">No hay compras POS con estos filtros.</p>
  ...
  return (
    <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5 dark:border-slate-700 dark:bg-slate-900">
        <div>
          <h3 className="text-lg font-semibold text-slate-950 dark:text-white">Compras POS</h3>
        ...
        <div className="flex flex-wrap gap-2">
          <span className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-200">
              ...
              return (
                <tr key={order.id} className="align-top transition hover:bg-orange-50/50 dark:hover:bg-orange-500/5">
                  <td className="px-5 py-5">
                    ...
                    <p className="mt-3 font-semibold text-slate-900 dark:text-slate-100">{order.providerName}</p>
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{order.providerEmail || 'Sin email'}</p>
                  </td>
                  <td className="px-5 py-5">
                    <p className="font-semibold text-slate-900 dark:text-slate-100">{order.warehouseName}</p>
                    <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">Esperado: {formatDate(order.expectedDate)}</p>
                    <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">{order.items.length} partidas</p>
                  ...
                  <td className="px-5 py-5">
                    <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${inventoryStatus.className}`}>
                      {inventoryStatus.label}
                    </span>
                    <p className="mt-2 font-bold text-slate-950 dark:text-white">{receivedQuantity} recibidas</p>
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{pendingQuantity} pendientes</p>
                  </td>
                  <td className="px-5 py-5">
                    {primaryInvoice ? (
                      <div>
                        <p className="max-w-[220px] truncate font-bold text-slate-950 dark:text-white">{primaryInvoice.invoiceNumber}</p>
                        <p className="mt-1 text-xs font-semibold text-slate-500 dark:text-slate-400">
                          {formatMoney(primaryInvoice.totalAmount, primaryInvoice.currencyCode)}
                        </p>
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${statusClassName(primaryInvoice.status)}`}>
                            {paymentStatus}
                          </span>
                          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Vence: {formatDate(primaryInvoice.dueDate)}</span>
                        </div>
                        {orderInvoices.length > 1 ? (
                          <span className="mt-2 inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-200">
                      ...
                      {primaryInvoice ? (
                        <>
                          {primaryInvoice.documentUrl ? (
                            <a href={primaryInvoice.documentUrl} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200 dark:hover:bg-slate-800">
...
function IconAction({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`inline-flex h-9 items-center gap-2 rounded-xl border px-3 text-xs font-bold transition disabled:cursor-not-allowed disabled:opacity-50 ${toneClass}`}
    >
