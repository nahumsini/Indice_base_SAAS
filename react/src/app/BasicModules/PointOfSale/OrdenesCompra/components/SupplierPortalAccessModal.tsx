...
function PortalQr({ alt, errorMessage, linkLabel, url }: {
  ...
  return (
    <div className="flex min-h-32 w-full flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-950 sm:w-32">
      {failed ? (
        <p role="status" className="px-2 text-center text-xs font-medium text-red-600 dark:text-red-300">{errorMessage}</p>
      ) : dataUrl ? (
    ...
    return (
      ...
      >
        <div className="space-y-3">
          {actionError ? <p role="alert" className="text-sm font-medium text-red-700 dark:text-red-200">{actionError}</p> : null}
          <label className="block space-y-2">
            <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{copy.admin.deleteReason}</span>
            <textarea
              value={deleteReason}
              maxLength={500}
      ...
      >
        <div className="grid min-h-0 bg-slate-50 dark:bg-slate-950 xl:grid-cols-[360px_minmax(0,1fr)]">
          <section className="border-b border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900 sm:p-6 xl:border-b-0 xl:border-r">
            ...
            {actionError ? (
              <div role="alert" className="mt-4 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-medium text-red-700 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-200">
                <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
                {actionError}
              </div>
            ) : null}
            {engineError ? (
              <div role="status" className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-medium text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-100">
                <p>{engineError}</p>
                <button type="button" disabled={engineLoading} onClick={() => void loadKiosks()} className="mt-2 min-h-9 rounded-lg border border-amber-300 bg-white px-3 font-medium text-amber-900 disabled:opacity-60 dark:border-amber-800 dark:bg-slate-950 dark:text-amber-100">
            ...
            <label className="mt-5 block space-y-2">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{copy.admin.supplier}</span>
              <select
                value={providerId}
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
              ...
              {!neverExpires ? (
                <input
                  ...
                  className="mt-3 h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-950 outline-none focus-visible:ring-2 focus-visible:ring-orange-400 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
                />
              ) : null}
          ...
          <section className="min-w-0 space-y-4 p-4 sm:p-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-medium text-slate-950 dark:text-white">{copy.admin.accessList}</h2>
            ...
            {accessList.length === 0 ? (
              <div className="rounded-[20px] border border-dashed border-slate-300 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-900">
                <p className="font-medium text-slate-950 dark:text-white">{copy.admin.emptyTitle}</p>
              ...
              return (
                <article key={access.id} className="rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-5">
                  <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_128px]">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <h3 className="font-medium text-slate-950 dark:text-white">{kiosk?.name || access.providerName}</h3>
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusClassName[accessStatus]}`}>
                          {copy.common.status[accessStatus]}
                        </span>
                        {kiosk ? <span className="rounded-full bg-sky-50 px-2 py-0.5 text-xs font-medium text-sky-700 dark:bg-sky-500/10 dark:text-sky-200">{copy.admin.engineBadge}</span> : null}
                      </div>
                      <p className="mt-1 text-sm font-medium text-slate-700 dark:text-slate-200">{access.providerName}</p>
                      <p className="mt-1 text-xs font-medium text-slate-500 dark:text-slate-400">
                        {kiosk?.publicTokenHint ? copy.admin.protectedLink(kiosk.publicTokenHint) : copy.admin.linkHidden}
                      </p>
                      {portalUrl ? <p className="mt-1 break-all text-xs font-medium text-orange-700 dark:text-orange-200">{portalUrl}</p> : null}
                      <p className="mt-2 text-xs font-medium text-slate-500 dark:text-slate-400">{formatExpiration(kiosk?.expiresAt ?? access.expiresAt)}</p>
                      {kiosk ? (
                        ...
                        <p className="mt-2 text-xs font-medium text-amber-700 dark:text-amber-200">{copy.admin.advancedUnavailable}</p>
                      ) : null}

                      {credentialOutcome === 'reused' ? (
                        <div role="status" className="mt-3 rounded-2xl border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-medium text-sky-800 dark:border-sky-900/50 dark:bg-sky-950/40 dark:text-sky-100">
                          {copy.admin.reusedCredential}
                        </div>
                      ) : null}
                      {revealedPin ? (
                        <div role="status" className="mt-3 flex flex-wrap items-center gap-2 rounded-2xl border border-[#FFB3AD] bg-[#FFF1EF] px-3 py-2 text-sm font-medium text-[#B63B32] dark:border-[#FF6B5E]/40 dark:bg-[#FF6B5E]/10 dark:text-[#FFC7C3]">
                    ...
                    {portalUrl ? (
                      ...
                      <div className="flex min-h-32 w-full flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-3 text-center dark:border-slate-700 dark:bg-slate-950 sm:w-32">
                        ...
                        <p className="mt-2 text-[11px] font-medium leading-4 text-slate-500 dark:text-slate-400">{copy.admin.linkHidden}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4 dark:border-slate-800">
                    ...
                    {accessStatus === 'ACTIVE' ? (
                      <button type="button" disabled={saving || Boolean(pendingAction)} onClick={() => void changeStatus(access, 'DISABLED')} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 px-3 text-xs font-medium text-slate-700 outline-none hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-orange-400 disabled:opacity-60 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                        <Power className="h-4 w-4" />
                        {copy.admin.disable}
                      </button>
                    ) : null}
                    {accessStatus === 'DISABLED' ? (
                      <button type="button" disabled={saving || Boolean(pendingAction)} onClick={() => void changeStatus(access, 'ACTIVE')} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-emerald-200 px-3 text-xs font-medium text-emerald-700 outline-none hover:bg-emerald-50 focus-visible:ring-2 focus-visible:ring-emerald-400 disabled:opacity-60 dark:border-emerald-900/50 dark:text-emerald-200 dark:hover:bg-emerald-950/30">
                        <Power className="h-4 w-4" />
                        {copy.admin.enable}
                      </button>
                    ) : null}
                  ...
                  {editingAccessId === access.id && kiosk ? (
                    <section className="mt-4 rounded-2xl border border-sky-200 bg-sky-50/70 p-4 dark:border-sky-900/50 dark:bg-sky-950/20" aria-label={copy.admin.configuration}>
                      <div className="grid gap-4 md:grid-cols-2">
                        <label className="block space-y-2">
                          <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{copy.admin.kioskName}</span>
                          <input value={editName} maxLength={180} onChange={event => setEditName(event.target.value)} className="h-11 w-full rounded-xl border border-sky-200 bg-white px-3 text-sm font-medium text-slate-950 outline-none focus-visible:ring-2 focus-visible:ring-sky-400 dark:border-sky-800 dark:bg-slate-950 dark:text-white" />
                        </label>
                        <div>
                          ...
                          {!editNeverExpires ? (
                            <input type="datetime-local" aria-label={copy.admin.validity} value={editExpiresAt} min={toLocalDateTime(new Date().toISOString())} onChange={event => setEditExpiresAt(event.target.value)} className="h-11 w-full rounded-xl border border-sky-200 bg-white px-3 text-sm font-medium text-slate-950 outline-none focus-visible:ring-2 focus-visible:ring-sky-400 dark:border-sky-800 dark:bg-slate-950 dark:text-white" />
                          ) : null}
                        </div>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button type="button" disabled={!editName.trim() || Boolean(pendingAction)} onClick={() => void saveConfiguration(access, kiosk)} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-sky-700 px-4 text-xs font-medium text-white disabled:opacity-60">
                          {pendingAction === `config-${access.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
                          {copy.admin.saveConfiguration}
                        </button>
                        <button type="button" disabled={Boolean(pendingAction)} onClick={() => setEditingAccessId(null)} className="min-h-10 rounded-xl border border-sky-200 bg-white px-4 text-xs font-medium text-sky-800 disabled:opacity-60 dark:border-sky-800 dark:bg-slate-950 dark:text-sky-200">{copy.common.cancel}</button>
                      </div>
                    </section>
                  ) : null}
                  ...
                  {expandedAccessId === access.id && kiosk ? (
                    <section className="mt-4 grid gap-4 rounded-2xl border border-violet-200 bg-violet-50/60 p-4 dark:border-violet-900/50 dark:bg-violet-950/20 lg:grid-cols-2" aria-label={copy.admin.details}>
                      {details?.loading ? (
                        <div className="col-span-full flex min-h-24 items-center justify-center gap-2 text-sm font-medium text-violet-700 dark:text-violet-200">
                        ...
                        <>
                          <div>
                            <h4 className="flex items-center gap-2 text-sm font-medium text-slate-950 dark:text-white"><Users className="h-4 w-4" />{copy.admin.grants}</h4>
                            <div className="mt-2 space-y-2">
                              {details?.grants.length ? details.grants.map(grant => (
                                <div key={grant.id} className="rounded-xl border border-violet-100 bg-white p-3 text-xs dark:border-violet-900/40 dark:bg-slate-950">
                                  <div className="flex flex-wrap items-center justify-between gap-2">
                                    <span className="font-medium text-slate-900 dark:text-white">{copy.admin.providerGrant} #{grant.identityId}</span>
                                    <span className={grant.status === 'ACTIVE' ? 'font-medium text-emerald-600 dark:text-emerald-300' : 'font-medium text-red-600 dark:text-red-300'}>{grant.status}</span>
                                  </div>
                              ...
                              )) : <p className="rounded-xl border border-dashed border-violet-200 p-3 text-xs font-medium text-slate-500 dark:border-violet-900/50 dark:text-slate-400">{copy.admin.noGrants}</p>}
                            </div>
                          </div>
                          <div>
                            <h4 className="flex items-center gap-2 text-sm font-medium text-slate-950 dark:text-white"><History className="h-4 w-4" />{copy.admin.audit}</h4>
                            <div className="mt-2 max-h-64 space-y-2 overflow-y-auto pr-1">
                              {details?.audit.length ? details.audit.map(event => (
                                <div key={`${event.eventId}-${event.createdAt}`} className="rounded-xl border border-violet-100 bg-white p-3 text-xs dark:border-violet-900/40 dark:bg-slate-950">
                                  <div className="flex flex-wrap items-center justify-between gap-2">
                                    ...
                                    <span className="font-medium text-violet-700 dark:text-violet-200">{event.outcome}</span>
                                  </div>
                              ...
                              )) : <p className="rounded-xl border border-dashed border-violet-200 p-3 text-xs font-medium text-slate-500 dark:border-violet-900/50 dark:text-slate-400">{copy.admin.noAudit}</p>}
                            </div>
                          </div>
                        </>
                      )}
