...
const mapClosingToAuditRecord = (
  ...
  return {
    ...
    auditStatus,
    businessId: String(row.cashRegisterId),
    businessName: `Caja ${row.cashRegisterId}`,
    businessUnitId: String(row.warehouseId),
    businessUnitName: `Almacén ${row.warehouseId}`,
    cardCounted: 0,
    cardExpected: 0,
    cashCounted: toNumber(row.countedCashAmount),
    cashExpected: toNumber(row.expectedCashAmount),
    cashRegisterCode: `Caja ${row.cashRegisterId}`,
    cashRegisterId: String(row.cashRegisterId),
    cashRegisterName: `Caja ${row.cashRegisterId}`,
    companyId: 'current',
    ...
    responsibleUserName: `Usuario ${row.closedByUserId}`,
