    ...
    <div className="space-y-5">
      <header className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <nav className="mt-5 flex flex-wrap gap-2" aria-label={copy.workspace.navigationLabel}>
          <button type="button" onClick={() => setSection('self-service')} className={`inline-flex h-10 items-center gap-2 rounded-lg px-4 text-sm font-medium ${section === 'self-service' ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-700 dark:bg-slate-900 dark:text-slate-200'}`}><ShoppingBasket className="h-4 w-4" />{copy.workspace.selfServiceTab}</button>
          <button type="button" onClick={() => setSection('customer-display')} className={`inline-flex h-10 items-center gap-2 rounded-lg px-4 text-sm font-medium ${section === 'customer-display' ? 'bg-[#FF6B5E] text-[#222831]' : 'bg-slate-100 text-slate-700 dark:bg-slate-900 dark:text-slate-200'}`}><Monitor className="h-4 w-4" />{copy.workspace.customerDisplayTab}</button>
        </nav>
      </header>
