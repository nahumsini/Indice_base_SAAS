...
function ProgressLine({ danger = false, value }: { danger?: boolean; value: number }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="w-12 text-right text-sm font-bold text-slate-900 dark:text-white">{percent(value)}</span>
    </div>
  );
}
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-4 flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h3 className="text-lg font-bold text-slate-950 dark:text-white">Senales ejecutivas POS</h3>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-lg border border-slate-200 p-5 dark:border-slate-800">
          <div className="mb-3 flex items-center justify-between">
            <p className="font-bold text-slate-700 dark:text-slate-200">Precision de caja</p>
            <Scale className="h-5 w-5 text-[#B63B32] dark:text-[#FFB0AA]" />
          </div>
          <p className="mb-4 text-3xl font-bold text-slate-950 dark:text-white">{percent(cashAccuracy)}</p>
          <ProgressLine value={cashAccuracy} danger={hasRisk} />
        </div>

        <div className="rounded-lg border border-slate-200 p-5 dark:border-slate-800">
          <div className="mb-3 flex items-center justify-between">
            <p className="font-bold text-slate-700 dark:text-slate-200">Tickets cerrados</p>
            <Ticket className="h-5 w-5 text-emerald-600 dark:text-emerald-300" />
          </div>
          <p className="mb-4 text-3xl font-bold text-slate-950 dark:text-white">{analytics.tickets}</p>
          <p className="text-sm text-slate-500 dark:text-slate-400">{analytics.closings} cierres alimentan la lectura.</p>
        </div>

        <div className="rounded-lg border border-slate-200 p-5 dark:border-slate-800">
          <div className="mb-3 flex items-center justify-between">
            <p className="font-bold text-slate-700 dark:text-slate-200">Lectura ejecutiva</p>
