  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <section>
          <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Tipo de descuento</p>
            ...
            <div className="space-y-2">
              {eligibleRules.slice(0, 4).map((rule) => (
                ...
                >
                  ...
                  <span className="shrink-0 rounded-lg bg-white px-2 py-1 text-xs font-medium text-orange-700 dark:bg-gray-950/40 dark:text-orange-300">
        ...
        {discountType === 'percentage' ? (
          <section>
            <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Descuentos rapidos</p>
            <div className="grid grid-cols-4 gap-2">
              {[5, 10, 15, 20].map((value) => (
                <button
                  ...
                  className="min-h-11 rounded-lg bg-gray-100 px-3 py-2 text-sm font-medium text-gray-900 transition hover:bg-gray-200 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600"
                >
                  {value}%
                </button>
              ))}
        ...
        <section className="rounded-lg border-2 border-[#FF6B5E] bg-[#FF6B5E]/10 p-4">
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between border-t border-[#FF6B5E]/40 pt-2 text-lg font-medium text-[#222831] dark:text-white">
...
function PreviewRow({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className={`flex justify-between font-medium ${accent ? 'text-[#A7352C] dark:text-[#FFB5AE]' : 'text-gray-700 dark:text-gray-300'}`}>
      <span>{label}:</span>
      <span>{value}</span>
    </div>
  );
