...
// The marketing website's new Mexico rates are not yet verifiable: /planes.php
// redirects to the older USD signup catalog. Never substitute an invented FX rate.
export const commercialPlanPrices: Record<CommercialPlan['id'], {
  monthlyMxn: number | null;
  annualMxn: number | null;
}> & { includedSeats: number; communityImplementationMxn: number; communityImplementationPeople: number } = {
  controla: { monthlyMxn: null, annualMxn: null },
  escala: { monthlyMxn: null, annualMxn: null },
  corporate: { monthlyMxn: null, annualMxn: null },
  includedSeats: 10,
  communityImplementationMxn: 4500,
  communityImplementationPeople: 10,
} as const;
...
const es: CommercialPresentationCopy = {
  ...
  pricing: {
    ...
    lead: 'Tres paquetes con ERP, agentes y acompañamiento. Incluyen 10 personas; la capacidad crece en bloques de diez y cada persona cuenta una sola vez.',
    ...
    offerLabel: 'Oferta especial · Comunidad Master Muñoz',
    offerTitle: 'Implementación para un equipo de hasta 10 personas.',
    offerDescription: 'Creada a petición de Carlos Muñoz para que su comunidad pueda comenzar con estructura, capacitación y acompañamiento desde el primer día.',
    offerPriceLabel: 'Pago único de implementación',
    ...
    offerCondition: 'La suscripción mensual del paquete elegido se contrata por separado. Precio antes de IVA y sujeto a confirmación de alcance.',
...
const en: CommercialPresentationCopy = {
  ...
  pricing: { eyebrow: '05 · Investment', title: 'Clear plans to start and grow.', lead: 'Three packages with ERP, agents, and guidance. Ten people included; capacity grows in blocks of ten and each person counts only once.', plansTitle: 'Monthly packages for Mexico', plans: [{ id: 'controla', title: 'Control', description: 'Organize the team and turn commitments into follow-up.', includes: ['Home Panel and company structure', 'Human Resources', 'Tasks and Processes'] }, { id: 'escala', title: 'Scale', description: 'Connect people, money, and commercial operations.', includes: ['Everything in Control', 'Expenses and Petty Cash', 'Sales or POS with Inventory'] }, { id: 'corporate', title: 'Corporate', description: 'Integrate the full operation and broaden executive insight.', includes: ['Everything in Scale, with Sales and POS together', 'Receivables', 'Executive KPI module'] }], monthlyLabel: 'per month', pendingPriceLabel: 'To be confirmed', annualLabel: 'Annual plan', beforeTaxLabel: 'Prices in MXN before VAT', commonTitle: 'Every package includes', commonItems: ['10 people', 'Lupita and agents based on modules and permissions', 'One 60-minute consulting session each month', 'Learning Mode'], offerLabel: 'Special offer · Master Muñoz Community', offerTitle: 'Implementation for a team of up to 10 people.', offerDescription: 'Created at Carlos Muñoz’s request so his community can start with structure, training, and guidance from day one.', offerPriceLabel: 'One-time implementation fee', offerIncludesTitle: 'Includes', offerIncludes: ['Assessment and scope definition', 'ERP and authorized agent configuration', 'Setup and training for up to 10 people', 'Activation and launch guidance'], offerCondition: 'The selected package subscription is contracted separately. Price is before VAT and subject to scope confirmation.', next: 'A defined investment to organize operations now and expand capabilities as the company grows.' },
...
const pt: CommercialPresentationCopy = {
  ...
  pricing: { ...en.pricing, eyebrow: '05 · Investimento', title: 'Planos claros para começar e crescer.', lead: 'Três pacotes com ERP, agentes e acompanhamento. Dez pessoas incluídas; a capacidade cresce em blocos de dez e cada pessoa conta uma única vez.', plansTitle: 'Pacotes mensais para o México', plans: [{ id: 'controla', title: 'Controle', description: 'Organiza a equipe e transforma compromissos em acompanhamento.', includes: ['Painel Inicial e estrutura empresarial', 'Recursos Humanos', 'Tarefas e Processos'] }, { id: 'escala', title: 'Escala', description: 'Conecta pessoas, dinheiro e operação comercial.', includes: ['Tudo do Controle', 'Despesas e Caixa Pequeno', 'Vendas ou PDV com Estoque'] }, { id: 'corporate', title: 'Corporativo', description: 'Integra toda a operação e amplia a visão executiva.', includes: ['Tudo do Escala, com Vendas e PDV juntos', 'Contas a receber', 'Módulo executivo de KPIs'] }], monthlyLabel: 'por mês', pendingPriceLabel: 'A confirmar', annualLabel: 'Plano anual', beforeTaxLabel: 'Preços em MXN antes do IVA', commonTitle: 'Todos os pacotes incluem', commonItems: ['10 pessoas', 'Lupita e agentes conforme módulos e permissões', 'Uma consultoria mensal de 60 minutos', 'Modo Aprendiz'], offerLabel: 'Oferta especial · Comunidade Master Muñoz', offerTitle: 'Implementação para uma equipe de até 10 pessoas.', offerDescription: 'Criada a pedido de Carlos Muñoz para que sua comunidade comece com estrutura, capacitação e acompanhamento.', offerPriceLabel: 'Pagamento único de implementação', offerIncludesTitle: 'Inclui', offerIncludes: ['Diagnóstico e definição de escopo', 'Configuração do ERP e agentes autorizados', 'Cadastro e treinamento de até 10 pessoas', 'Ativação e acompanhamento inicial'], offerCondition: 'A assinatura do pacote escolhido é contratada separadamente. Preço antes do IVA e sujeito à confirmação do escopo.', next: 'Um investimento definido para organizar a operação e ampliar capacidades conforme a empresa cresce.' },
...
const ko: CommercialPresentationCopy = {
  ...
  pricing: { ...en.pricing, eyebrow: '05 · 투자', title: '시작과 성장을 위한 명확한 요금제.', lead: 'ERP, 에이전트, 지원을 결합한 세 패키지에 10명이 포함됩니다. 인원은 10명 단위로 확장하며 각 사람은 한 번만 계산합니다.', plansTitle: '멕시코 월간 패키지', plans: [{ id: 'controla', title: 'Controla', description: '팀을 정리하고 약속을 후속 관리로 전환합니다.', includes: ['초기 패널과 기업 구조', '인사 관리', '업무와 프로세스'] }, { id: 'escala', title: 'Escala', description: '인력, 자금, 상업 운영을 연결합니다.', includes: ['Controla 전체', '비용과 소액 현금', '판매 또는 POS와 재고'] }, { id: 'corporate', title: 'Corporativo', description: '전체 운영을 통합하고 경영 시야를 넓힙니다.', includes: ['Escala 전체와 판매 및 POS 모두', '미수금', '경영 KPI 모듈'] }], monthlyLabel: '월', pendingPriceLabel: '확인 예정', annualLabel: '연간 요금제', beforeTaxLabel: '부가세 별도 MXN 가격', commonTitle: '모든 패키지 포함 사항', commonItems: ['10명 포함', '모듈과 권한에 따른 Lupita 및 에이전트', '매월 60분 컨설팅 1회', '학습 모드'], offerLabel: '특별 혜택 · Master Muñoz 커뮤니티', offerTitle: '최대 10명을 위한 도입 서비스.', offerDescription: 'Carlos Muñoz의 요청으로 커뮤니티가 첫날부터 체계, 교육, 지원을 갖추도록 마련했습니다.', offerPriceLabel: '일회성 도입 비용', offerIncludesTitle: '포함 사항', offerIncludes: ['진단과 범위 정의', 'ERP와 승인된 에이전트 설정', '최대 10명 등록 및 교육', '활성화와 초기 정착 지원'], offerCondition: '선택한 패키지 구독료는 별도입니다. 부가세 별도이며 범위 확인이 필요합니다.', next: '운영을 정리하고 성장에 따라 역량을 확대하기 위한 명확한 투자입니다.' },
...
const zh: CommercialPresentationCopy = {
  ...
  pricing: { ...en.pricing, eyebrow: '05 · 投资', title: '清晰的方案，支持起步与成长。', lead: '三个套餐均包含 ERP、智能体和顾问支持，涵盖 10 人；按每组 10 人扩展，每人仅计数一次。', plansTitle: '墨西哥月度套餐', plans: [{ id: 'controla', title: 'Controla', description: '组织团队，并将承诺转化为持续跟进。', includes: ['初始面板与企业结构', '人力资源', '任务与流程'] }, { id: 'escala', title: 'Escala', description: '连接人员、资金与商业运营。', includes: ['Controla 全部内容', '费用与备用金', '销售或 POS 与库存'] }, { id: 'corporate', title: 'Corporativo', description: '整合完整运营并拓展管理视角。', includes: ['Escala 全部内容，同时包含销售和 POS', '应收账款', '管理 KPI 模块'] }], monthlyLabel: '每月', pendingPriceLabel: '待确认', annualLabel: '年度套餐', beforeTaxLabel: 'MXN 价格，不含增值税', commonTitle: '所有套餐均包含', commonItems: ['10 人', '根据模块与权限启用 Lupita 及智能体', '每月一次 60 分钟咨询', '学习模式'], offerLabel: '特别优惠 · Master Muñoz 社区', offerTitle: '最多 10 人团队的实施服务。', offerDescription: '应 Carlos Muñoz 的请求设立，让其社区从第一天起获得结构、培训与支持。', offerPriceLabel: '一次性实施费用', offerIncludesTitle: '包含', offerIncludes: ['诊断与范围定义', 'ERP 与授权智能体配置', '最多 10 人开通与培训', '启用与上线陪伴'], offerCondition: '所选套餐订阅费另行签约。价格不含增值税，并须确认实施范围。', next: '以明确投入整理当前运营，并随企业成长扩展能力。' },
