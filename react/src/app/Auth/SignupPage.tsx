...
const brandSelectClasses = 'h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none focus:border-[var(--indice-brand-aqua)] focus:ring-2 focus:ring-[var(--indice-brand-aqua)]/25';
...
export default function SignupPage() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(89,195,165,0.16),_transparent_34%),linear-gradient(135deg,_#F8FAFC_0%,_#EEF3F8_55%,_#F8FAFC_100%)] px-4 py-6 text-[#222831] sm:px-6 lg:py-10">
      <div className="mx-auto max-w-6xl">
        <nav className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <Link to="/login" className="text-xl font-black tracking-tight text-[var(--indice-brand-action)]">Índice</Link>
            <Button asChild variant="ghost" className="h-10 rounded-full px-3 text-sm font-bold text-slate-600 hover:bg-white/80 hover:text-[var(--indice-structural-blue)]">
          ...
          <div className="flex flex-wrap items-center justify-end gap-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button aria-label={copy.languageLabel} variant="outline" className="h-10 gap-2 rounded-full border-slate-200 bg-white/90 px-4 text-[var(--indice-brand-action)] shadow-sm">
            ...
            <Button asChild variant="outline" className="h-10 rounded-full border-slate-200 bg-white/90 px-4 text-sm font-bold text-slate-700 shadow-sm hover:border-[var(--indice-brand-aqua)] hover:bg-white hover:text-[var(--indice-brand-action)]">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.78fr_1.22fr]">
          <aside className="rounded-[32px] bg-[var(--indice-brand-action)] p-7 text-white shadow-[0_30px_80px_-42px_rgba(23,125,102,0.58)] lg:sticky lg:top-10 lg:self-start lg:p-9">
            ...
            <div className="mt-8 space-y-4 text-sm font-semibold">
              <div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#F4C84A]" /><span>{copy.ownerBullet}</span></div>
              <div className="flex gap-3"><Users className="mt-0.5 h-5 w-5 shrink-0 text-white" /><span>{copy.usersBullet}</span></div>
              <div className="flex gap-3"><CreditCard className="mt-0.5 h-5 w-5 shrink-0 text-[#FF6B5E]" /><span>{copy.stripeBullet}</span></div>
            </div>
            {billingStep ? (
              <div className="mt-9 rounded-2xl border border-white/15 bg-white/10 p-5">
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-white/75">{copy.afterTrial}</p>
                <p className="mt-2 text-3xl font-black">{currency(visibleSignupEstimate, form.billingInterval, copy)}</p>
                {form.billingInterval === 'YEAR' ? <p className="mt-1 text-sm text-white/75">{copy.annualSavings}</p> : null}
              </div>
            ) : null}
          </aside>
          ...
          <section className="rounded-[32px] border border-white/80 bg-white/95 p-5 shadow-[0_28px_80px_-50px_rgba(34,40,49,0.45)] sm:p-8">
            <div className="flex items-start gap-4 border-b border-slate-100 pb-6">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--indice-brand-soft)] text-[var(--indice-brand-action)]"><Building2 className="h-6 w-6" /></span>
              <div>
                <p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--indice-brand-action)]">{billingStep ? copy.billingBadge : copy.accountBadge}</p>
              ...
              accountDetailsComplete ? (
                <form className="mt-6 space-y-7" onSubmit={submit}>
                    ...
                    {planCards.map((card) => {
                      ...
                      return (
                        <button
                          ...
                          className={`min-h-40 rounded-2xl border bg-white p-4 text-left transition ${selected ? 'border-[var(--indice-brand-aqua)] bg-[var(--indice-brand-soft)] shadow-sm' : 'border-slate-200 hover:border-[var(--indice-brand-aqua)] hover:bg-slate-50'}`}
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-center gap-3">
                              <Layers3 className={`h-5 w-5 ${selected ? 'text-[var(--indice-brand-action)]' : 'text-slate-400'}`} />
                              <h3 className="text-lg font-black tracking-tight text-slate-900">{card.title}</h3>
                            </div>
                            {selected ? <CheckCircle2 className="h-5 w-5 shrink-0 text-[var(--indice-brand-action)]" /> : null}
                          </div>
                      ...
                      {config?.products.map((product) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex min-h-[52px] items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-left text-base font-black transition ${selected ? 'border-[var(--indice-brand-aqua)] bg-[var(--indice-brand-soft)] text-slate-900 shadow-sm' : 'border-slate-200 bg-white text-slate-600 hover:border-[var(--indice-brand-aqua)] hover:bg-slate-50'}`}
                          >
                            {productLabel(product.code, product.displayName, copy)}
                            {selected ? (
                              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-[var(--indice-brand-action)] text-[var(--indice-brand-action)]"><Check className="h-3.5 w-3.5" /></span>
                            ) : null}
                          </button>
                        );
                      })}
                    ...
                    <div className="mt-4 grid gap-4 sm:grid-cols-2">
                      <div className="grid grid-cols-2 rounded-2xl bg-slate-100 p-1">
                        {(['MONTH', 'YEAR'] as const).map((interval) => (
                          <button key={interval} type="button" onClick={() => update('billingInterval', interval)} className={`h-11 rounded-xl text-sm font-bold transition ${form.billingInterval === interval ? 'bg-white text-[var(--indice-brand-action)] shadow-sm' : 'text-slate-500'}`}>
                  ...
                  <div className="grid gap-3 sm:grid-cols-[0.8fr_1.2fr]">
                    ...
                    <Button type="submit" disabled={!canSubmit} className="h-13 rounded-xl bg-[var(--indice-brand-action)] text-base font-black text-white hover:bg-[var(--indice-brand-action-hover)]">
                ...
                <div className="mt-6 space-y-4">
                  ...
                  <Button type="button" onClick={() => navigate('/signup')} className="h-13 w-full rounded-xl bg-[var(--indice-brand-action)] text-base font-black text-white hover:bg-[var(--indice-brand-action-hover)]">
                    <ArrowLeft className="h-5 w-5" /> {copy.accountDetailsButton}
                  </Button>
                </div>
              )
              ...
              <form className="mt-6 space-y-7" onSubmit={continueToBilling}>
                <fieldset className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="space-y-2 text-sm font-bold text-slate-700">{copy.companyLabel}
                    <Input value={form.companyName} onChange={(event) => update('companyName', event.target.value)} maxLength={120} className={brandInputClasses} placeholder={copy.companyPlaceholder} autoComplete="organization" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">{copy.ownerLabel}
                    <Input value={form.fullName} onChange={(event) => update('fullName', event.target.value)} maxLength={100} className={brandInputClasses} placeholder={copy.ownerPlaceholder} autoComplete="name" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">{copy.emailLabel}
                    <Input type="email" value={form.email} onChange={(event) => update('email', event.target.value)} maxLength={190} className={brandInputClasses} placeholder={copy.emailPlaceholder} autoComplete="email" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">{copy.passwordLabel}
                    <Input type="password" value={form.password} onChange={(event) => update('password', event.target.value)} className={brandInputClasses} placeholder={copy.passwordPlaceholder} autoComplete="new-password" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">{copy.countryLabel}
                    <select value={form.countryCode} onChange={(event) => update('countryCode', event.target.value)} className={brandSelectClasses}>
                ...
                <Button type="submit" className="h-13 w-full rounded-xl bg-[var(--indice-brand-action)] text-base font-black text-white hover:bg-[var(--indice-brand-action-hover)]">
