    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(89,195,165,0.16),_transparent_34%),linear-gradient(135deg,_#F8FAFC_0%,_#EEF3F8_55%,_#F8FAFC_100%)] px-4 py-6 text-[#222831] sm:px-6 lg:py-10">
      <div className="mx-auto max-w-6xl">
        <nav className="mb-6 flex items-center justify-between gap-4">
          ...
          <Link to="/login" className="text-sm font-bold text-slate-600 transition hover:text-[var(--indice-structural-blue)]">
            Ya tengo una cuenta
          </Link>
        </nav>
        ...
        <div className="grid gap-6 lg:grid-cols-[0.78fr_1.22fr]">
          <aside className="rounded-[32px] bg-[var(--indice-brand-action)] p-7 text-white shadow-[0_30px_80px_-42px_rgba(23,125,102,0.58)] lg:sticky lg:top-10 lg:self-start lg:p-9">
            ...
            <div className="mt-8 space-y-4 text-sm font-semibold">
              <div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#F4C84A]" /><span>Tarjeta requerida y cobro automático al terminar la prueba.</span></div>
              <div className="flex gap-3"><Users className="mt-0.5 h-5 w-5 shrink-0 text-white" /><span>5 usuarios incluidos; agrega los que tu operación necesite.</span></div>
              <div className="flex gap-3"><CreditCard className="mt-0.5 h-5 w-5 shrink-0 text-[#FF6B5E]" /><span>Precios en USD antes de impuestos. Stripe procesa tus datos de pago.</span></div>
            </div>
            {estimatedAmount !== null ? (
              <div className="mt-9 rounded-2xl border border-white/15 bg-white/10 p-5">
                <p className="text-xs font-bold uppercase tracking-[0.14em] text-white/75">Después de la prueba</p>
                <p className="mt-2 text-3xl font-black">{currency(estimatedAmount, form.billingInterval)}</p>
                {form.billingInterval === 'YEAR' ? <p className="mt-1 text-sm text-white/75">20% de ahorro anual incluido.</p> : null}
              </div>
            ) : null}
          </aside>
          ...
          <section className="rounded-[32px] border border-white/80 bg-white/95 p-5 shadow-[0_28px_80px_-50px_rgba(34,40,49,0.45)] sm:p-8">
            <div className="flex items-start gap-4 border-b border-slate-100 pb-6">
              ...
              <div>
                <p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--indice-brand-action)]">Crea tu cuenta corporativa</p>
              ...
              <form className="mt-6 space-y-7" onSubmit={submit}>
                    ...
                    {config?.products.map((product) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex min-h-14 items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-left text-sm font-bold transition ${selected ? 'border-[var(--indice-brand-aqua)] bg-[var(--indice-brand-soft)] text-[var(--indice-brand-action)] shadow-sm' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300'}`}
                        >
                          {product.displayName}
                          <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border ${selected ? 'border-[var(--indice-brand-action)] bg-[var(--indice-brand-action)] text-white' : 'border-slate-300 text-transparent'}`}><Check className="h-4 w-4" /></span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <fieldset className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="space-y-2 text-sm font-bold text-slate-700">Marca corporativa
                    <Input value={form.companyName} onChange={(event) => update('companyName', event.target.value)} maxLength={120} className={brandInputClasses} placeholder="Nombre de tu empresa" autoComplete="organization" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">Nombre del propietario
                    <Input value={form.fullName} onChange={(event) => update('fullName', event.target.value)} maxLength={100} className={brandInputClasses} placeholder="Nombre completo" autoComplete="name" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">Correo principal
                    <Input type="email" value={form.email} onChange={(event) => update('email', event.target.value)} maxLength={190} className={brandInputClasses} placeholder="tu@empresa.com" autoComplete="email" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">Contraseña
                    <Input type="password" value={form.password} onChange={(event) => update('password', event.target.value)} className={brandInputClasses} placeholder="Mínimo 10 caracteres" autoComplete="new-password" />
                  </label>
                  <label className="space-y-2 text-sm font-bold text-slate-700">País de lanzamiento
                    <select value={form.countryCode} onChange={(event) => update('countryCode', event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none focus:border-[var(--indice-brand-aqua)] focus:ring-2 focus:ring-[var(--indice-brand-aqua)]/25">
                  ...
                  <label className="space-y-2 text-sm font-bold text-slate-700">Teléfono <span className="font-medium text-slate-400">(opcional)</span>
                    <Input value={form.phone} onChange={(event) => update('phone', event.target.value)} maxLength={40} className={brandInputClasses} placeholder="+52…" autoComplete="tel" />
                  </label>
                </fieldset>
                ...
                <Button type="submit" disabled={!canSubmit} className="h-13 w-full rounded-xl bg-[var(--indice-brand-action)] text-base font-black text-white hover:bg-[var(--indice-brand-action-hover)]">
