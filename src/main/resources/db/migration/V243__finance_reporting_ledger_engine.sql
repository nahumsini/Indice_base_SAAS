-- Financial reporting ledger engine. Version 243 follows the adopted V234-V242 sequence.
ALTER TABLE finance_accounting_accounts
  ADD COLUMN parent_account_id BIGINT NULL AFTER business_id,
  ADD COLUMN system_code VARCHAR(80) NULL AFTER code,
  ADD COLUMN account_type VARCHAR(32) NOT NULL DEFAULT 'EXPENSE' AFTER description,
  ADD COLUMN natural_balance VARCHAR(16) NOT NULL DEFAULT 'DEBIT' AFTER account_type,
  ADD COLUMN is_postable BOOLEAN NOT NULL DEFAULT TRUE AFTER natural_balance,
  ADD COLUMN statement_section VARCHAR(64) NULL AFTER is_postable,
  ADD COLUMN statement_line_code VARCHAR(80) NULL AFTER statement_section,
  ADD COLUMN sort_order INT NOT NULL DEFAULT 0 AFTER statement_line_code,
  ADD UNIQUE KEY uq_fin_accounts_company_system_code (company_id, system_code),
  ADD KEY idx_fin_accounts_company_type (company_id, account_type, status),
  ADD KEY idx_fin_accounts_parent (parent_account_id),
  ADD CONSTRAINT fk_fin_accounts_parent
    FOREIGN KEY (parent_account_id) REFERENCES finance_accounting_accounts(id) ON DELETE SET NULL,
  ADD CONSTRAINT chk_fin_accounts_type
    CHECK (account_type IN ('ASSET', 'LIABILITY', 'EQUITY', 'REVENUE', 'EXPENSE', 'OCI')),
  ADD CONSTRAINT chk_fin_accounts_natural_balance
    CHECK (natural_balance IN ('DEBIT', 'CREDIT'));

CREATE TABLE finance_accounting_settings (
...
CREATE TABLE finance_financial_report_runs (
  id BIGINT NOT NULL AUTO_INCREMENT,
  company_id BIGINT NOT NULL,
  period_start DATE NOT NULL,
  period_end DATE NOT NULL,
