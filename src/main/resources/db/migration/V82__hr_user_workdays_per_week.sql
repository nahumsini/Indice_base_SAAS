SET @add_user_work_profile_workdays_per_week := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `user_work_profiles` ADD COLUMN `workdays_per_week` decimal(4,2) NOT NULL DEFAULT ''5.00'' AFTER `workday_hours`',
    'SELECT 1'
  )
  FROM information_schema.columns
  WHERE table_schema = DATABASE()
    AND table_name = 'user_work_profiles'
    AND column_name = 'workdays_per_week'
);

PREPARE stmt FROM @add_user_work_profile_workdays_per_week;
EXECUTE stmt;
