...
- Fresh empty databases can now be built from the latest baseline migration, currently `B22__spring_backend_baseline.sql`.
...
- `B22__spring_backend_baseline.sql` is the cumulative baseline for the current Spring-owned subset of the schema through `V22`.
...
- The next normal versioned migration should start at `V24__...`.
