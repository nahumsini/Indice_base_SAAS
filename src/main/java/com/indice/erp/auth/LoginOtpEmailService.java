...
public class LoginOtpEmailService {
    ...
    private DeliveryResult sendWithSmtp(AuthenticatedLogin login, String otpCode, int ttlSeconds) {
        ...
        try {
            ...
            message.setText(body(login, otpCode, ttlSeconds));
    ...
    private Map<String, Object> sendGridPayload(AuthenticatedLogin login, String otpCode, int ttlSeconds) {
        ...
        payload.put("content", java.util.List.of(Map.of("type", "text/plain", "value", body(login, otpCode, ttlSeconds))));
        return payload;
    }

    private String body(AuthenticatedLogin login, String otpCode, int ttlSeconds) {
        var minutes = Math.max(1, ttlSeconds / 60);
        return """
            Hi %s,

            Use this verification code to finish signing in to %s:
