...
@RestController
@RequestMapping("/api/v1/finance/expenses/{expenseId}/attachments")
public class FinanceExpenseAttachmentController {

    private final FinanceRequestGuard guard;
    private final ExpenseAttachmentService service;

    public FinanceExpenseAttachmentController(FinanceRequestGuard guard, ExpenseAttachmentService service) {
        this.guard = guard;
        this.service = service;
    }

    @GetMapping
    public ResponseEntity<?> list(HttpSession session, @PathVariable long expenseId) {
        var access = guard.requireReadAccess(session);
        if (access.denied()) {
            return access.error();
        }
        try {
            return ResponseEntity.ok(service.list(access.context(), expenseId));
    ...
    public ResponseEntity<?> presignUpload(
        ...
        try {
            return ResponseEntity.ok(service.presignUpload(access.context(), expenseId, request));
        } catch (NoSuchElementException ex) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", ex.getMessage()));
    ...
    public ResponseEntity<?> register(
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(service.register(access.context(), expenseId, request));
        } catch (NoSuchElementException ex) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", ex.getMessage()));
        } catch (ObjectStorageDisabledException ex) {
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(Map.of("message", ex.getMessage()));
    ...
    public ResponseEntity<?> delete(
        ...
        try {
            service.delete(access.context(), expenseId, attachmentId);
