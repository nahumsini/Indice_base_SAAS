...
prepare_backend_env() {
...

  {
    ...
    if [[ -n "${APP_POS_MERCADO_PAGO_APPLICATION_SECRET_FILE:-}" ]]; then
      printf 'APP_POS_MERCADO_PAGO_APPLICATION_SECRET_FILE=%s\n' "${APP_POS_MERCADO_PAGO_APPLICATION_SECRET_FILE}"
    fi
    if [[ -n "${APP_POS_MERCADO_PAGO_WEBHOOK_SECRET_FILE:-}" ]]; then
      printf 'APP_POS_MERCADO_PAGO_WEBHOOK_SECRET_FILE=%s\n' "${APP_POS_MERCADO_PAGO_WEBHOOK_SECRET_FILE}"
    fi
    if [[ -n "${APP_POS_MERCADO_PAGO_TOKEN_PROTECTION_SECRET_FILE:-}" ]]; then
      printf 'APP_POS_MERCADO_PAGO_TOKEN_PROTECTION_SECRET_FILE=%s\n' "${APP_POS_MERCADO_PAGO_TOKEN_PROTECTION_SECRET_FILE}"
    fi
  } >>"${output}"
}
...
prepare_backend_secret_mount APP_POS_MERCADO_PAGO_APPLICATION_SECRET_FILE /run/secrets/indice-mp-application-secret
prepare_backend_secret_mount APP_POS_MERCADO_PAGO_WEBHOOK_SECRET_FILE /run/secrets/indice-mp-webhook-secret
